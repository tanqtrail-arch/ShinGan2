// 真贋 (ShinGan) - 絵画鑑定ゲーム データ
// 9ステージ × 9問 = 81問

const STAGES = [
  // ===== Stage 1: ルネサンス =====
  {
    id: 1,
    name: "ルネサンス",
    period: "14〜16世紀",
    description:
      "14世紀イタリアに始まり16世紀に最盛期を迎えた芸術運動。古代ギリシャ・ローマの古典文化の復興を目指し、人間中心主義（ヒューマニズム）を根底に据えました。線遠近法による空間表現、人体解剖学に基づく正確な人物描写、スフマート技法による柔らかな色調遷移が特徴です。レオナルド・ダ・ヴィンチ、ミケランジェロ、ラファエロの三大巨匠が代表的存在です。",
    palette: ["#5C4033", "#8B7355", "#D4C5A9", "#2F4F4F", "#8B6914"],
    questions: [
      {
        title: "モナ・リザ",
        artist: "レオナルド・ダ・ヴィンチ",
        year: "1503-1519年",
        medium: "ポプラ板に油彩",
        size: "77cm × 53cm",
        location: "ルーヴル美術館",
        description:
          "微笑みを浮かべる女性の半身像。スフマート技法による柔らかな色調遷移と、背景の幻想的な山岳風景が特徴的。輪郭線を用いない革新的な表現手法で描かれている。",
        isAuthentic: true,
        hint: "ダ・ヴィンチ特有のスフマート技法に注目してください。",
        explanation:
          "本物です。レオナルド・ダ・ヴィンチの代表作で、スフマート技法（輪郭を煙のようにぼかす手法）が完璧に用いられています。ポプラ板に油彩で描かれ、現在ルーヴル美術館に所蔵されています。",
        colors: ["#5C4033", "#8B7355", "#D4C5A9", "#2F4F4F", "#1a1a2e"],
        fakeExplanation:
          "微笑みが原作の神秘的で曖昧な微笑から、はっきりとしたニヤリとした笑みに変えられています。また背景の色調が全体的に緑がかって鮮やかになっており、ダ・ヴィンチ特有のスフマート技法による霞がかった淡い色調とは異なります。",
        realImage: "images/s1-q1-real.jpg",
        fakeImage: "images/s1-q1-fake.jpg",
      },
      {
        title: "ヴィーナスの誕生",
        artist: "サンドロ・ボッティチェリ",
        year: "1485年頃",
        medium: "カンヴァスにテンペラ",
        size: "172.5cm × 278.9cm",
        location: "ウフィツィ美術館",
        description:
          "貝殻の上に立つヴィーナスが風に吹かれて岸に運ばれる場面。流麗な線描と淡い色彩で、古典神話の美を理想的に表現している。",
        isAuthentic: true,
        hint: "ボッティチェリの線描表現と使用されている画材に注目。",
        explanation:
          "本物です。ボッティチェリがテンペラ画で描いた傑作。当時カンヴァスにテンペラで大作を描くこと自体が革新的でした。優美な線描はボッティチェリの真骨頂です。",
        colors: ["#E8D5B7", "#87CEEB", "#F5DEB3", "#DDA0DD", "#90EE90"],
        fakeExplanation:
          "背景が本来の明るい海と空から夜空に変更され、月や星が追加されています。ボッティチェリの原作にはない幻想的な夜景が描き加えられた贋作です。",
        realImage: "images/s1-q2-real.jpg",
        fakeImage: "images/s1-q2-fake.jpg",
      },
      {
        title: "最後の晩餐",
        artist: "レオナルド・ダ・ヴィンチ",
        year: "1495-1498年",
        medium: "壁面にテンペラと油彩の混合技法",
        size: "460cm × 880cm",
        location: "サンタ・マリア・デッレ・グラツィエ修道院",
        description:
          "キリストが弟子たちに裏切りを予告する劇的瞬間を描いた壁画。一点透視図法により奥行きが強調され、各弟子の感情表現が卓越している。",
        isAuthentic: true,
        hint: "遠近法の使い方と技法に注目してください。",
        explanation:
          "本物です。ダ・ヴィンチは伝統的なフレスコ技法ではなく、テンペラと油彩の実験的混合技法を用いました。この技法ゆえに劣化が早く進みましたが、これは史実と一致します。",
        colors: ["#8B7355", "#CD853F", "#D2B48C", "#556B2F", "#8B4513"],
        fakeExplanation:
          "テーブルの上にピザやファストフードが置かれ、弟子たちがスマートフォンを持っているなど、明らかに現代的な要素が描き加えられた改竄作品です。",
        realImage: "images/s1-q3-real.jpg",
        fakeImage: "images/s1-q3-fake.jpg",
      },
      {
        title: "アテナイの学堂",
        artist: "ラファエロ・サンツィオ",
        year: "1509-1511年",
        medium: "壁面にフレスコ",
        size: "500cm × 770cm",
        location: "バチカン宮殿",
        description:
          "古代ギリシャの哲学者たちが壮大な建築空間に集う場面。中央にプラトンとアリストテレスが配され、完璧な遠近法で描かれた建築が奥行きを生んでいる。",
        isAuthentic: true,
        hint: "ラファエロの構図の完璧さと建築表現を観察してください。",
        explanation:
          "本物です。ラファエロがバチカン宮殿の「署名の間」に描いたフレスコ画。古代の哲学者たちの姿に、同時代の芸術家（ダ・ヴィンチ、ミケランジェロなど）の容貌を重ねたことでも知られています。",
        colors: ["#D4C5A9", "#CD853F", "#87CEEB", "#DEB887", "#BC8F8F"],
        fakeExplanation:
          "額縁が原作にはない重厚な装飾付きの木製額縁に変更されています。また全体の色調がやや暖色寄りに変質しており、フレスコ画本来の色味とは異なります。画像の縁がぼやけた処理が施されている点も不自然です。",
        realImage: "images/s1-q4-real.jpg",
        fakeImage: "images/s1-q4-fake.jpg",
      },
      {
        title: "聖母子と二人の天使",
        artist: "フィリッポ・リッピ",
        year: "1460-1465年頃",
        medium: "板にテンペラ",
        size: "95cm × 63.5cm",
        location: "ウフィツィ美術館",
        description:
          "聖母マリアが幼子キリストを抱き、二人の天使が支える構図。窓の向こうに広がる風景、聖母の透明感ある肌と繊細なヴェールの描写が美しい。手前の天使が微笑みながら鑑賞者を見つめている。",
        isAuthentic: true,
        hint: "リッピの聖母像の特徴と、テンペラ画の質感に注目。",
        explanation:
          "本物です。初期ルネサンスの巨匠リッピの代表作。聖母のモデルは修道女ルクレツィア・ブーティとされ、彼女との恋愛は有名な逸話です。弟子ボッティチェリに大きな影響を与えた優美な聖母像の傑作。ウフィツィ美術館所蔵。",
        colors: ["#E8D5B7", "#87CEEB", "#D4C5A9", "#DEB887", "#8B7355"],
        fakeExplanation:
          "背景がサイバーパンク風のネオン輝く未来都市に完全に差し替えられています。聖母と天使の光輪が電子回路の模様に変わり、15世紀イタリアの宗教画にはありえないSF的な世界観が混入しています。",
        realImage: "images/s1-q5-real.jpg",
        fakeImage: "images/s1-q5-fake.jpg",
      },
      {
        title: "東方三博士の礼拝",
        artist: "サンドロ・ボッティチェリ",
        year: "1475年頃",
        medium: "板にテンペラ",
        size: "111cm × 134cm",
        location: "ウフィツィ美術館",
        description:
          "キリスト誕生を祝う東方三博士の礼拝を描いた宗教画。メディチ家の人々を三博士や随行者として描き込み、右端にはボッティチェリ自身の自画像も配されている。優美な人物配置と淡い色彩が特徴。",
        isAuthentic: true,
        hint: "描かれた人物の中に実在の人物が含まれていないか注目。",
        explanation:
          "本物です。ボッティチェリがメディチ家の後援のもとに描いた作品。コジモ・デ・メディチ、ロレンツォ豪華王らが三博士やその従者として描かれ、右端でこちらを見つめる金髪の青年がボッティチェリ自身です。テンペラによる淡く優美な色彩はボッティチェリの真骨頂です。",
        colors: ["#DAA520", "#8B4513", "#D4C5A9", "#CD853F", "#556B2F"],
        fakeExplanation:
          "聖母マリアが抱いている幼子キリストが白い猫に置き換えられています。本来は幼子イエスを抱く厳粛な宗教画ですが、猫という全く異質な要素が紛れ込んだユーモラスな改竄です。",
        realImage: "images/s1-q6-real.jpg",
        fakeImage: "images/s1-q6-fake.jpg",
      },
      {
        title: "アダムの創造",
        artist: "ミケランジェロ・ブオナローティ",
        year: "1508-1512年",
        medium: "天井にフレスコ",
        size: "280cm × 570cm",
        location: "バチカン市国 システィーナ礼拝堂",
        description:
          "神の右手とアダムの左手が触れ合おうとする瞬間を描いた天井画。神は天使たちに囲まれて飛来し、横たわるアダムへ生命を吹き込む。筋肉の躍動感と指先の緊張感が圧倒的。",
        isAuthentic: true,
        hint: "ミケランジェロは彫刻家としても有名。人体表現に注目を。",
        explanation:
          "本物です。システィーナ礼拝堂天井画の中で最も象徴的な場面。ミケランジェロの彫刻家としての解剖学知識が、アダムの理想化された肉体に反映されています。神とアダムの指先がわずかに離れた構図は、西洋美術史上最も有名なイメージの一つです。",
        colors: ["#D4C5A9", "#CD853F", "#87CEEB", "#E8D5B7", "#A0522D"],
        fakeExplanation:
          "神とアダムの指先が触れ合い、接触部分から光が放たれています。原作では指先は「わずかに離れている」ことが最大の特徴であり、触れ合っていません。この有名な「間」が失われている点が決定的な違いです。",
        realImage: "images/s1-q7-real.jpg",
        fakeImage: "images/s1-q7-fake.jpg",
      },
      {
        title: "レヴィ家の饗宴",
        artist: "パオロ・ヴェロネーゼ",
        year: "1573年",
        medium: "カンヴァスに油彩",
        size: "555cm × 1,280cm",
        location: "アカデミア美術館（ヴェネツィア）",
        description:
          "壮大な古典建築を背景に、豪華な宴席の場面が展開する大作。兵士、道化師、犬、異国の人々が入り交じる華やかで賑やかな構図。鮮やかな色彩とヴェネツィア派の光の表現が特徴。",
        isAuthentic: true,
        hint: "この作品は当初別の題名で描かれ、宗教裁判で問題になりました。",
        explanation:
          "本物です。元々「最後の晩餐」として描かれましたが、宗教的場面にふさわしくない人物（道化師、犬、酔った兵士など）を描いたとして異端審問にかけられました。ヴェロネーゼは題名を「レヴィ家の饗宴」に変更することで問題を回避した逸話が有名です。",
        colors: ["#DAA520", "#87CEEB", "#CD853F", "#F5F5DC", "#8B4513"],
        fakeExplanation:
          "背景の建築が大きく改変され、白い尖塔や見慣れない建物が追加されています。テーブル中央に原作にはない大きな金色の球体が置かれ、全体の空が深い青色に変わっています。構図も縦長にトリミングされ、原作の壮大な横長の構図が失われています。",
        realImage: "images/s1-q8-real.jpg",
        fakeImage: "images/s1-q8-fake.jpg",
      },
      {
        title: "ナスタージョ・デリ・オネスティの物語 第一話",
        artist: "サンドロ・ボッティチェリ",
        year: "1483年頃",
        medium: "板にテンペラ",
        size: "83cm × 138cm",
        location: "プラド美術館",
        description:
          "ボッカッチョ『デカメロン』第五日第八話を題材にした連作の第一場面。松林の中で失恋に苦しむナスタージョが、騎士に追われる裸の女性の幻影に遭遇する劇的な瞬間を描く。",
        isAuthentic: true,
        hint: "ボッティチェリの物語画の構成と文学的題材に注目。",
        explanation:
          "本物です。メディチ家の婚礼を祝う注文作品として描かれた4枚連作の第一場面。ボッティチェリの流麗な線描で物語の劇的場面が表現されています。文学作品の視覚化という点でルネサンス絵画の知性を示す作品。プラド美術館所蔵。",
        colors: ["#228B22", "#8B4513", "#87CEEB", "#DEB887", "#556B2F"],
        fakeExplanation:
          "松林の色調や人物の配置に改変が加えられています。ボッティチェリの流麗な線描による物語表現が不自然に変更されています。",
        realImage: "images/s1-q9-real.jpg",
        fakeImage: "images/s1-q9-fake.jpg",
      },
    ],
  },

  // ===== Stage 2: バロック =====
  {
    id: 2,
    name: "バロック",
    period: "17世紀",
    description:
      "16世紀末から18世紀初頭にかけてヨーロッパで栄えた様式。カトリック教会の対抗宗教改革を背景に、劇的な光と影のコントラスト（キアロスクーロ）、壮大な構図、激しい感情表現が特徴です。カラヴァッジョの革命的な明暗法、レンブラントの深い精神性、ベラスケスの写実性、フェルメールの静謐な光の表現など、個性的な巨匠たちが活躍しました。",
    palette: ["#2C1810", "#8B4513", "#DAA520", "#F5F5DC", "#1a1a2e"],
    questions: [
      {
        title: "夜警",
        artist: "レンブラント・ファン・レイン",
        year: "1642年",
        medium: "カンヴァスに油彩",
        size: "363cm × 437cm",
        location: "アムステルダム国立美術館",
        description:
          "フランス・バニング・コック隊長率いる市民自警団の集団肖像画。劇的な光が中央の人物たちを照らし出し、周囲は深い闇に沈む。動きと活気に満ちた構図が革新的。",
        isAuthentic: true,
        hint: "レンブラント特有の光の使い方に注目してください。",
        explanation:
          "本物です。レンブラントの最高傑作の一つ。集団肖像画でありながら、劇的なキアロスクーロ（明暗法）で物語性を持たせた革新的な作品です。実際には「夜」の場面ではなく、ニスの変色で暗く見えるようになりました。",
        colors: ["#1a1a2e", "#DAA520", "#8B4513", "#2C1810", "#F5F5DC"],
        fakeExplanation:
          "光と影のコントラストに違和感があり、中央人物を照らす劇的な光の配置がレンブラントの原作と微妙に異なっています。",
        realImage: "images/s2-q1-real.jpg",
        fakeImage: "images/s2-q1-fake.jpg",
      },
      {
        title: "真珠の耳飾りの少女",
        artist: "ヨハネス・フェルメール",
        year: "1665年頃",
        medium: "カンヴァスに油彩",
        size: "44.5cm × 39cm",
        location: "マウリッツハイス美術館",
        description:
          "ターバンを巻いた少女が肩越しに振り返る瞬間を捉えた肖像画。暗い背景に浮かび上がる少女の顔と、光を受けて輝く真珠の耳飾りが印象的。",
        isAuthentic: true,
        hint: "フェルメール特有の光の捉え方と色使いに注目。",
        explanation:
          "本物です。フェルメールの代表作「北のモナ・リザ」とも呼ばれます。ウルトラマリン（ラピスラズリ由来）の青と鉛錫黄の対比、精緻な光の表現がフェルメールの真作であることを示しています。",
        colors: ["#00008B", "#FFD700", "#F5F5DC", "#1a1a2e", "#4682B4"],
        fakeExplanation:
          "少女のターバンの色や真珠の光り方に改変が見られます。フェルメール特有のウルトラマリンブルーの深みが不自然に変更されています。",
        realImage: "images/s2-q2-real.jpg",
        fakeImage: "images/s2-q2-fake.jpg",
      },
      {
        title: "聖マタイの召命",
        artist: "カラヴァッジョ",
        year: "1599-1600年",
        medium: "カンヴァスに油彩",
        size: "322cm × 340cm",
        location: "サン・ルイジ・デイ・フランチェージ聖堂",
        description:
          "キリストが徴税人マタイを弟子に召す場面。右上から差し込む劇的な光が暗い室内を切り裂き、当時の衣装をまとう人物たちの表情を浮かび上がらせる。",
        isAuthentic: true,
        hint: "カラヴァッジョの光と影の使い方は唯一無二です。",
        explanation:
          "本物です。カラヴァッジョの明暗法の革新を示す傑作。聖書の場面を当時の居酒屋のような場所に設定し、登場人物に同時代の衣装を着せるという斬新な手法は、バロック絵画に革命をもたらしました。",
        colors: ["#1a1a2e", "#8B4513", "#DAA520", "#2C1810", "#CD853F"],
        fakeExplanation:
          "光の差し込む角度や人物たちの衣装の色彩が原作と異なります。カラヴァッジョの劇的な明暗法の特徴が微妙に損なわれています。",
        realImage: "images/s2-q3-real.jpg",
        fakeImage: "images/s2-q3-fake.jpg",
      },
      {
        title: "ラス・メニーナス",
        artist: "ディエゴ・ベラスケス",
        year: "1656年",
        medium: "カンヴァスに油彩",
        size: "318cm × 276cm",
        location: "プラド美術館",
        description:
          "スペイン王女マルガリータを中心に、侍女たち、画家自身、鏡に映る国王夫妻を配した複雑な構図。空間と視線の巧みな操作で絵画の本質を問いかける。",
        isAuthentic: true,
        hint: "ベラスケスの空間表現と光の使い方に注目。",
        explanation:
          "本物です。ベラスケスの最高傑作であり、西洋絵画史上最も分析された作品の一つ。鏡の反射、画家自身の自画像、複数の視点が交錯する複雑な構造は、絵画の可能性を拡張しました。",
        colors: ["#696969", "#D4C5A9", "#8B7355", "#2F4F4F", "#A0522D"],
        fakeExplanation:
          "鏡に映る人物や空間の遠近法に不自然な改変があります。ベラスケスの精密な空間構成が微妙にズレています。",
        realImage: "images/s2-q4-real.jpg",
        fakeImage: "images/s2-q4-fake.jpg",
      },
      {
        title: "デルフトの夜景",
        artist: "ヨハネス・フェルメール",
        year: "1658年",
        medium: "カンヴァスに油彩",
        size: "72cm × 88cm",
        location: "ウィーン個人蔵",
        description:
          "月明かりに照らされたデルフトの街並み。フェルメール特有の青色が夜空に広がり、運河に映る建物が幻想的に描かれている。プルシアンブルーの深い青が印象的。",
        isAuthentic: false,
        hint: "使用されている顔料が当時存在したか確認してください。",
        explanation:
          "贋作です。プルシアンブルー（紺青）は1704年にベルリンで偶然発見された顔料であり、1658年にフェルメールが使用することは不可能です。当時の青はウルトラマリンやアズライトに限られていました。",
        colors: ["#191970", "#000080", "#4169E1", "#1a1a2e", "#C0C0C0"],
        fakeExplanation:
          "色調や街並みの配置に改変が見られます。プルシアンブルーの使用など、時代にそぐわない顔料が使われた形跡があります。",
        realImage: "images/s2-q5-real.jpg",
        fakeImage: "images/s2-q5-fake.jpg",
      },
      {
        title: "聖パウロの回心",
        artist: "カラヴァッジョ",
        year: "1601年",
        medium: "板にテンペラ",
        size: "230cm × 175cm",
        location: "ナポリ個人蔵",
        description:
          "落馬するパウロの劇的な瞬間を板にテンペラで描いた作品。均一で明るい光が画面全体を照らし、人物はやや平面的に表現されている。繊細な線描で輪郭が丁寧に描かれている。",
        isAuthentic: false,
        hint: "カラヴァッジョが好んだ画材と技法を思い出してください。",
        explanation:
          "贋作です。カラヴァッジョは板にテンペラではなく、カンヴァスに油彩で描くことを基本としました。また「均一で明るい光」「平面的」「繊細な線描」はすべてカラヴァッジョの画風（劇的な明暗、立体感、大胆な筆致）と正反対です。",
        colors: ["#F5F5DC", "#DEB887", "#D2B48C", "#FAEBD7", "#CD853F"],
        fakeExplanation:
          "人物の表情や光の当たり方に違和感があります。カラヴァッジョの特徴的な明暗対比が不自然に再現されています。",
        realImage: "images/s2-q6-real.jpg",
        fakeImage: "images/s2-q6-fake.jpg",
      },
      {
        title: "テュルプ博士の解剖学講義",
        artist: "レンブラント・ファン・レイン",
        year: "1632年",
        medium: "カンヴァスに油彩",
        size: "169.5cm × 216.5cm",
        location: "マウリッツハイス美術館",
        description:
          "テュルプ博士が公開解剖を行う場面。博士が死体の腕の筋肉を示し、周囲の外科医たちがそれぞれ異なる反応を見せる。光が解剖台の上に集中している。",
        isAuthentic: true,
        hint: "レンブラントの集団肖像画の特徴を考えてみてください。",
        explanation:
          "本物です。レンブラント26歳の出世作。集団肖像画でありながら、解剖という「事件」を通じて各人物に異なる表情と動きを与えた画期的な作品です。光源の処理もレンブラント特有のものです。",
        colors: ["#1a1a2e", "#2C1810", "#F5F5DC", "#8B7355", "#696969"],
        fakeExplanation:
          "解剖台の照明や周囲の人物の配置に微妙な変更が加えられています。レンブラントの光源処理に不自然な点があります。",
        realImage: "images/s2-q7-real.jpg",
        fakeImage: "images/s2-q7-fake.jpg",
      },
      {
        title: "放蕩息子の帰還",
        artist: "レンブラント・ファン・レイン",
        year: "1668-1669年頃",
        medium: "カンヴァスに油彩",
        size: "262cm × 205cm",
        location: "エルミタージュ美術館",
        description:
          "ぼろぼろの衣服をまとった息子が父の前にひざまずき、老いた父が両手で息子の背を包み込む。深い闇の中に人物が浮かび上がり、赦しと慈愛の瞬間が静かに描かれている。",
        isAuthentic: true,
        hint: "レンブラント晩年の精神性と光の使い方に注目。",
        explanation:
          "本物です。レンブラント最晩年の傑作。新約聖書の「放蕩息子」のたとえ話を題材に、赦しと和解の普遍的テーマを深い精神性で描いています。晩年特有の厚塗りの筆致と、闇の中に浮かぶ光の表現はレンブラント芸術の到達点です。エルミタージュ美術館所蔵。",
        colors: ["#1a1a2e", "#8B4513", "#DAA520", "#2C1810", "#CD853F"],
        fakeExplanation:
          "父と子の手の位置関係や衣服の色調に改変が見られます。レンブラント晩年の厚塗り技法の質感が不自然に変更されています。",
        realImage: "images/s2-q8-real.jpg",
        fakeImage: "images/s2-q8-fake.jpg",
      },
      {
        title: "牛乳を注ぐ女",
        artist: "ヨハネス・フェルメール",
        year: "1658-1660年頃",
        medium: "カンヴァスに油彩",
        size: "45.5cm × 41cm",
        location: "アムステルダム国立美術館",
        description:
          "窓からの光の中で牛乳を注ぐ女性。静謐な空間に差し込む光が、パンや陶器の質感を精緻に描き出している。青と黄の対比が画面に調和をもたらす。",
        isAuthentic: true,
        hint: "フェルメールの光と質感表現に注目してください。",
        explanation:
          "本物です。フェルメールの代表作の一つ。窓から差し込む自然光による静謐な室内表現、パンの質感を表現するためのポワンティエ（点描的技法）の使用、ウルトラマリンブルーと鉛錫黄の対比はフェルメールの真作の証です。",
        colors: ["#4682B4", "#FFD700", "#F5F5DC", "#8B7355", "#DEB887"],
        fakeExplanation:
          "窓からの光の角度や牛乳の注がれ方に微妙な違いがあります。フェルメール特有の光と質感の表現に不自然さが見られます。",
        realImage: "images/s2-q9-real.jpg",
        fakeImage: "images/s2-q9-fake.jpg",
      },
    ],
  },

  // ===== Stage 3: ロココ =====
  {
    id: 3,
    name: "ロココ",
    period: "18世紀",
    description:
      "18世紀フランス宮廷を中心に花開いた優美で装飾的な芸術様式。バロックの壮大さに対し、より軽快で繊細、親密な表現を追求しました。パステル調の淡い色彩、曲線的で流れるような構図、恋愛や牧歌的主題を好みます。ヴァトー、ブーシェ、フラゴナールがフランス・ロココの三大巨匠とされ、貴族社会の華やかさと退廃を映し出しました。",
    palette: ["#FFB6C1", "#E6E6FA", "#FFDAB9", "#98FB98", "#87CEEB"],
    questions: [
      {
        title: "シテール島への巡礼",
        artist: "アントワーヌ・ヴァトー",
        year: "1717年",
        medium: "カンヴァスに油彩",
        size: "129cm × 194cm",
        location: "ルーヴル美術館",
        description:
          "愛の島シテールへ向かう（または去る）恋人たちを描いた雅宴画。淡い色彩と詩的な雰囲気の中、カップルたちが夢幻的な風景を漂う。ヴァトーの代表作であり「雅宴画」ジャンルを確立した作品。",
        isAuthentic: true,
        hint: "ヴァトーが確立した「雅宴画」というジャンルに注目。",
        explanation:
          "本物です。ヴァトーがアカデミー入会作品として提出した傑作。恋愛の甘美さと儚さを同時に表現する「フェート・ギャラント（雅宴画）」というジャンルを生み出しました。ルーヴル美術館所蔵。",
        colors: ["#E6E6FA", "#FFB6C1", "#90EE90", "#FFDAB9", "#87CEEB"],
        fakeExplanation:
          "恋人たちの配置や風景の色調に改変が施されています。ヴァトーの詩的で夢幻的な雰囲気が微妙に損なわれています。",
        realImage: "images/s3-q1-real.jpg",
        fakeImage: "images/s3-q1-fake.jpg",
      },
      {
        title: "ぶらんこ",
        artist: "ジャン＝オノレ・フラゴナール",
        year: "1767年",
        medium: "カンヴァスに油彩",
        size: "81cm × 64cm",
        location: "ウォレス・コレクション",
        description:
          "茂みの中でぶらんこに乗る若い女性。片方の靴を飛ばす遊び心あふれるポーズで、下から見上げる若い男性と、後ろでぶらんこを押す老人という暗示的な構図。",
        isAuthentic: true,
        hint: "ロココ特有の遊び心と軽やかさに注目。",
        explanation:
          "本物です。フラゴナールの最も有名な作品。軽妙で官能的な主題、動きのある構図、バラ色の色調はロココ絵画の精華です。注文主の要望に応えた作品で、恋愛の駆け引きを優美に描いています。",
        colors: ["#FFB6C1", "#90EE90", "#FFC0CB", "#98FB98", "#DDA0DD"],
        fakeExplanation:
          "女性のポーズや周囲の茂みの色彩に変更があります。フラゴナールの軽快で官能的な表現が不自然に改変されています。",
        realImage: "images/s3-q2-real.jpg",
        fakeImage: "images/s3-q2-fake.jpg",
      },
      {
        title: "ポンパドゥール夫人の肖像",
        artist: "フランソワ・ブーシェ",
        year: "1756年",
        medium: "カンヴァスに油彩",
        size: "201cm × 157cm",
        location: "アルテ・ピナコテーク",
        description:
          "ルイ15世の寵姫ポンパドゥール夫人を華やかに描いた肖像画。豪華なドレス、花、書物に囲まれた夫人の知性と美を讃えている。柔らかな色彩と精密な布地の描写が見事。",
        isAuthentic: true,
        hint: "ブーシェの装飾的で華やかな表現を確認してください。",
        explanation:
          "本物です。ブーシェはポンパドゥール夫人お気に入りの宮廷画家で、複数の肖像を残しています。緻密な布地の描写、バラ色の肌、装飾的な構成はブーシェの特徴そのものです。",
        colors: ["#FFB6C1", "#E6E6FA", "#F0E68C", "#DDA0DD", "#FAEBD7"],
        fakeExplanation:
          "ドレスの装飾や背景の花の配置に微妙な変更が見られます。ブーシェの精密な布地描写のタッチが異なっています。",
        realImage: "images/s3-q3-real.jpg",
        fakeImage: "images/s3-q3-fake.jpg",
      },
      {
        title: "ヴェルサイユの大庭園",
        artist: "アントワーヌ・ヴァトー",
        year: "1720年",
        medium: "カンヴァスに油彩",
        size: "145cm × 200cm",
        location: "パリ個人蔵",
        description:
          "ヴェルサイユ宮殿の庭園を壮大に描いた風景画。幾何学的に整備された庭園を俯瞰的な視点から精密に記録している。建築的正確さを重視した硬質な筆致が特徴。",
        isAuthentic: false,
        hint: "ヴァトーの画風と、この絵の制作年に注意してください。",
        explanation:
          "贋作です。まず、ヴァトーは1721年に亡くなっており1720年は最晩年ですが、より重要なのは画風の矛盾です。ヴァトーは「建築的正確さ」や「硬質な筆致」とは正反対の、夢幻的で柔らかい雅宴画を得意としました。",
        colors: ["#556B2F", "#808080", "#A9A9A9", "#696969", "#2F4F4F"],
        fakeExplanation:
          "庭園の構図や色彩に改変が加えられています。ヴァトーの柔らかい雅宴画の画風とは異なる硬質な表現が混在しています。",
        realImage: "images/s3-q4-real.jpg",
        fakeImage: "images/s3-q4-fake.jpg",
      },
      {
        title: "読書する少女",
        artist: "ジャン＝オノレ・フラゴナール",
        year: "1769年頃",
        medium: "カンヴァスに油彩",
        size: "81.1cm × 64.8cm",
        location: "ナショナル・ギャラリー（ワシントン）",
        description:
          "クッションにもたれて読書に没頭する少女。レモンイエローのドレスが柔らかな光に映え、素早い筆致で描かれた自由闊達な表現が魅力的。",
        isAuthentic: true,
        hint: "フラゴナールの筆致の特徴に注目してください。",
        explanation:
          "本物です。フラゴナールの自由で素早い筆致が遺憾なく発揮された作品。レモンイエローの鮮やかさと、即興的な描写力はフラゴナール独特のものです。ワシントン・ナショナル・ギャラリー所蔵。",
        colors: ["#FFD700", "#FAFAD2", "#FFF8DC", "#FFB6C1", "#DEB887"],
        fakeExplanation:
          "ドレスの色や少女の表情に微妙な変更があります。フラゴナールの素早い筆致による自由闊達な表現に不自然さが見られます。",
        realImage: "images/s3-q5-real.jpg",
        fakeImage: "images/s3-q5-fake.jpg",
      },
      {
        title: "愛の寓意",
        artist: "フランソワ・ブーシェ",
        year: "1742年",
        medium: "カンヴァスに油彩",
        size: "117cm × 90cm",
        location: "リヨン個人蔵",
        description:
          "キューピッドとヴィーナスを描いた神話画。暗い背景にドラマティックな光が差し込み、人物は筋肉質に表現されている。重厚な色彩と厳格な構図が特徴。",
        isAuthentic: false,
        hint: "ブーシェの画風とロココの特徴を照らし合わせてください。",
        explanation:
          "贋作です。「暗い背景」「ドラマティックな光」「筋肉質」「重厚な色彩」「厳格な構図」はバロック様式の特徴であり、ロココ画家ブーシェの軽やかで装飾的、パステル調の画風とは正反対です。",
        colors: ["#1a1a2e", "#8B4513", "#2C1810", "#DAA520", "#696969"],
        fakeExplanation:
          "色彩や構図がロココの特徴から逸脱しています。ブーシェの軽やかで装飾的な画風とは異質な重厚さが混入しています。",
        realImage: "images/s3-q6-real.jpg",
        fakeImage: "images/s3-q6-fake.jpg",
      },
      {
        title: "ディアナの水浴",
        artist: "フランソワ・ブーシェ",
        year: "1742年",
        medium: "カンヴァスに油彩",
        size: "56cm × 73cm",
        location: "ルーヴル美術館",
        description:
          "狩りの女神ディアナが水浴する場面。淡いバラ色の肌、柔らかな光、牧歌的な風景の中で、理想化された女性美が表現されている。",
        isAuthentic: true,
        hint: "ブーシェの理想的な女性表現に注目してください。",
        explanation:
          "本物です。ブーシェの代表作の一つ。理想化された女性の肉体美、パステル調の色彩、装飾的な構成はロココ絵画の典型です。ルーヴル美術館に所蔵されています。",
        colors: ["#FFB6C1", "#E6E6FA", "#FFDAB9", "#90EE90", "#87CEEB"],
        fakeExplanation:
          "女性の肌の色調や背景の牧歌的風景に微妙な改変があります。ブーシェのパステル調の色彩感覚が変質しています。",
        realImage: "images/s3-q7-real.jpg",
        fakeImage: "images/s3-q7-fake.jpg",
      },
      {
        title: "マリー・アントワネットの庭園",
        artist: "ジャン＝オノレ・フラゴナール",
        year: "1785年",
        medium: "カンヴァスにアクリルガッシュ",
        size: "96cm × 130cm",
        location: "ヴェルサイユ宮殿美術館",
        description:
          "プチ・トリアノンの庭園でくつろぐマリー・アントワネットと侍女たち。鮮やかな発色のアクリルガッシュで、花々と人物が生き生きと表現されている。",
        isAuthentic: false,
        hint: "使用されている画材と時代の関係に注意してください。",
        explanation:
          "贋作です。アクリルガッシュは20世紀後半に開発された画材であり、18世紀のフラゴナールが使用することは不可能です。当時の画材は油彩、テンペラ、水彩、パステルなどに限られていました。",
        colors: ["#FF69B4", "#00FF7F", "#FF6347", "#4169E1", "#FFD700"],
        fakeExplanation:
          "花々の色彩や人物の衣装に不自然な鮮やかさがあります。18世紀の画材では出せない発色が含まれている贋作です。",
        realImage: "images/s3-q8-real.jpg",
        fakeImage: "images/s3-q8-fake.jpg",
      },
      {
        title: "田園の音楽会",
        artist: "アントワーヌ・ヴァトー",
        year: "1718年頃",
        medium: "カンヴァスに油彩",
        size: "65cm × 93cm",
        location: "ウォレス・コレクション",
        description:
          "木陰で音楽を奏でる貴族たち。淡い色調と詩的な光の中、人物たちは優雅に配置されている。ヴァトー特有の繊細な色彩感覚と憂愁の気配が漂う。",
        isAuthentic: true,
        hint: "ヴァトーの雅宴画の特徴を思い出してください。",
        explanation:
          "本物です。ヴァトーの雅宴画の典型的な作品。音楽、自然、恋愛、そしてどこか物悲しい雰囲気の融合はヴァトー独自のものです。彼の短い生涯（36歳で死去）の中で生み出された珠玉の一作。",
        colors: ["#E6E6FA", "#DEB887", "#90EE90", "#FFDAB9", "#D8BFD8"],
        fakeExplanation:
          "人物の配置や木陰の色調に微妙な変更が加えられています。ヴァトーの憂愁を帯びた詩的な雰囲気が損なわれています。",
        realImage: "images/s3-q9-real.jpg",
        fakeImage: "images/s3-q9-fake.jpg",
      },
    ],
  },

  // ===== Stage 4: 印象派 =====
  {
    id: 4,
    name: "印象派",
    period: "19世紀後半",
    description:
      "1860年代のフランスで生まれた革新的な芸術運動。戸外での制作を重視し、移ろう光と色彩の瞬間的な「印象」を捉えることを目指しました。絵具を混ぜずにカンヴァスに直接置く筆触分割、明るいパレット、日常的な主題が特徴です。モネ、ルノワール、ドガ、ピサロらが中心となり、アカデミーの古典的な規範に挑戦しました。",
    palette: ["#87CEEB", "#98FB98", "#FFD700", "#FF69B4", "#DDA0DD"],
    questions: [
      {
        title: "印象・日の出",
        artist: "クロード・モネ",
        year: "1872年",
        medium: "カンヴァスに油彩",
        size: "48cm × 63cm",
        location: "マルモッタン・モネ美術館",
        description:
          "ル・アーヴル港の日の出を素早い筆致で描いた作品。朝もやの中にオレンジの太陽が浮かび、水面に反射が揺らめく。形態よりも光と色彩の印象を優先した画法。",
        isAuthentic: true,
        hint: "この作品名が一つの芸術運動の名称になりました。",
        explanation:
          "本物です。「印象派」の名称の由来となった歴史的作品。1874年の第1回印象派展に出品され、批評家ルイ・ルロワが「印象に過ぎない」と揶揄したことから運動全体の名称となりました。",
        colors: ["#FF8C00", "#4682B4", "#87CEEB", "#FFD700", "#708090"],
        fakeExplanation:
          "太陽の色や水面の反射に改変が見られます。モネの素早い筆致による光の捉え方が不自然に変更されています。",
        realImage: "images/s4-q1-real.jpg",
        fakeImage: "images/s4-q1-fake.jpg",
      },
      {
        title: "ムーラン・ド・ラ・ギャレットの舞踏会",
        artist: "ピエール＝オーギュスト・ルノワール",
        year: "1876年",
        medium: "カンヴァスに油彩",
        size: "131cm × 175cm",
        location: "オルセー美術館",
        description:
          "モンマルトルの野外舞踏場での賑やかな日曜の午後。木漏れ日が踊る人々に斑模様の光を落とし、幸福感あふれる色彩で日常の喜びが表現されている。",
        isAuthentic: true,
        hint: "ルノワールの光と人物の描き方に注目してください。",
        explanation:
          "本物です。ルノワールの代表作。木漏れ日の斑点状の光、暖かみのある色彩、幸福感に満ちた人物描写はルノワールの真骨頂です。オルセー美術館の至宝の一つ。",
        colors: ["#FFD700", "#4169E1", "#FFB6C1", "#90EE90", "#DDA0DD"],
        fakeExplanation:
          "木漏れ日の表現や人物の色彩に微妙な違いがあります。ルノワールの暖かみのある幸福感あふれる色調が変質しています。",
        realImage: "images/s4-q2-real.jpg",
        fakeImage: "images/s4-q2-fake.jpg",
      },
      {
        title: "踊りの花形",
        artist: "エドガー・ドガ",
        year: "1876年頃",
        medium: "カンヴァスに油彩",
        size: "60cm × 44cm",
        location: "オルセー美術館",
        description:
          "舞台上でポーズを取るバレリーナ。高い視点から捉えた大胆な構図で、チュチュの白が舞台照明を受けて輝く。背景の踊り子たちがぼかされた処理で奥行きを演出。",
        isAuthentic: true,
        hint: "ドガの構図の取り方とバレエへの関心に注目。",
        explanation:
          "本物です。ドガはバレエを主題とした作品を多数残しました。写真的な瞬間の切り取り、斜めの視点、大胆なトリミングはドガの革新的な構図感覚を示しています。",
        colors: ["#FFFAF0", "#FFD700", "#FFB6C1", "#90EE90", "#F0E68C"],
        fakeExplanation:
          "バレリーナのポーズや舞台照明の表現に改変が施されています。ドガの斜めの視点による大胆な構図に不自然な変更があります。",
        realImage: "images/s4-q3-real.jpg",
        fakeImage: "images/s4-q3-fake.jpg",
      },
      {
        title: "パリの大通り、冬",
        artist: "クロード・モネ",
        year: "1873年",
        medium: "カンヴァスに油彩",
        size: "61cm × 80cm",
        location: "ブリュッセル個人蔵",
        description:
          "冬のパリの大通りを描いた都市風景。すべての建物が正確な設計図のように精密に描かれ、人物一人一人の顔の細部まで克明に表現されている。写真のような完璧な写実性が特徴。",
        isAuthentic: false,
        hint: "印象派の「印象」とは何を意味するか考えてみてください。",
        explanation:
          "贋作です。モネは「印象」を重視し、対象の精密な再現ではなく、光と色彩の瞬間的な効果を素早い筆致で捉えることを追求しました。「設計図のように精密」「写真のような写実性」はモネの画風と正反対です。",
        colors: ["#808080", "#A9A9A9", "#C0C0C0", "#D3D3D3", "#696969"],
        fakeExplanation:
          "建物や人物の描写に改変が見られます。モネの「印象」を重視した筆致とは異なる精密すぎる表現が混入しています。",
        realImage: "images/s4-q4-real.jpg",
        fakeImage: "images/s4-q4-fake.jpg",
      },
      {
        title: "睡蓮",
        artist: "クロード・モネ",
        year: "1906年",
        medium: "カンヴァスに油彩",
        size: "89.9cm × 94.1cm",
        location: "シカゴ美術館",
        description:
          "ジヴェルニーの庭園の池に浮かぶ睡蓮。水面に映る空と植物が溶け合い、筆触の重なりが光と色彩の交響曲を奏でる。具象と抽象の境界を漂う晩年の傑作。",
        isAuthentic: true,
        hint: "モネの晩年のライフワークを思い出してください。",
        explanation:
          "本物です。モネは晩年、ジヴェルニーの自宅庭園の睡蓮を主題に約250点もの連作を制作しました。水面への反射と睡蓮が一体化する表現は、後の抽象表現主義にも影響を与えました。",
        colors: ["#87CEEB", "#228B22", "#FFB6C1", "#4682B4", "#90EE90"],
        fakeExplanation:
          "睡蓮の配置や水面の反射に微妙な違いがあります。モネの筆触の重なりによる光と色彩の表現が不自然に変更されています。",
        realImage: "images/s4-q5-real.jpg",
        fakeImage: "images/s4-q5-fake.jpg",
      },
      {
        title: "セーヌ川の帆船",
        artist: "ピエール＝オーギュスト・ルノワール",
        year: "1874年",
        medium: "カンヴァスに油彩",
        size: "55cm × 65cm",
        location: "ジュネーブ個人蔵",
        description:
          "セーヌ川に浮かぶ帆船を描いた風景画。しかし画面全体がモノトーンの灰色で統一され、冷たく陰鬱な雰囲気が漂う。筆致は非常に細かく、滑らかな表面仕上げが施されている。",
        isAuthentic: false,
        hint: "ルノワールの色彩感覚と筆致の特徴を思い出してください。",
        explanation:
          "贋作です。ルノワールは暖かく豊かな色彩を愛した画家であり、「モノトーンの灰色」「冷たく陰鬱」は彼の画風と矛盾します。また印象派は素早く大胆な筆触を特徴とし、「滑らかな表面仕上げ」は印象派の技法に反します。",
        colors: ["#696969", "#808080", "#A9A9A9", "#778899", "#2F4F4F"],
        fakeExplanation:
          "色調や筆致に改変が見られます。ルノワールの暖かく豊かな色彩が不自然なモノトーンに変更されている贋作です。",
        realImage: "images/s4-q6-real.jpg",
        fakeImage: "images/s4-q6-fake.jpg",
      },
      {
        title: "サン＝ラザール駅",
        artist: "クロード・モネ",
        year: "1877年",
        medium: "カンヴァスに油彩",
        size: "75cm × 104cm",
        location: "オルセー美術館",
        description:
          "パリのサン＝ラザール駅構内を描いた作品。蒸気機関車の煙と蒸気が光を受けて虹色に輝き、鉄とガラスの駅舎が近代都市の活力を象徴している。",
        isAuthentic: true,
        hint: "モネが都市の近代性をどう捉えたか考えてみてください。",
        explanation:
          "本物です。モネは駅という近代的主題に挑み、蒸気と光の相互作用を見事に描きました。同じ駅を異なる角度や時間帯で12点の連作として制作。「連作」という手法の初期的な試みです。",
        colors: ["#4682B4", "#708090", "#87CEEB", "#B0C4DE", "#FFD700"],
        fakeExplanation:
          "蒸気や光の表現に微妙な違いがあります。モネの近代都市を捉える独自の視点と筆触が改変されています。",
        realImage: "images/s4-q7-real.jpg",
        fakeImage: "images/s4-q7-fake.jpg",
      },
      {
        title: "パリのカフェテラスの夜",
        artist: "エドガー・ドガ",
        year: "1880年",
        medium: "カンヴァスに油彩",
        size: "92cm × 73cm",
        location: "リヨン個人蔵",
        description:
          "夜のカフェテラスで語らう人々。点描法を用いて光の粒子が散りばめられ、色彩は原色のまま並置されている。科学的な色彩理論に基づいた厳密な色の配置が見られる。",
        isAuthentic: false,
        hint: "点描法（新印象派の技法）と印象派の違いを考えてください。",
        explanation:
          "贋作です。「点描法」「科学的な色彩理論」「厳密な色の配置」はスーラやシニャックの新印象派（点描主義）の特徴であり、ドガの技法とは異なります。ドガは素早いスケッチ的な筆致と独自の構図で知られています。",
        colors: ["#FF0000", "#0000FF", "#FFFF00", "#00FF00", "#FF8C00"],
        fakeExplanation:
          "色彩の配置や筆致に印象派とは異なる新印象派的な点描法が混入しています。ドガの技法との矛盾が見られます。",
        realImage: "images/s4-q8-real.jpg",
        fakeImage: "images/s4-q8-fake.jpg",
      },
      {
        title: "草上の昼食",
        artist: "エドゥアール・マネ",
        year: "1863年",
        medium: "カンヴァスに油彩",
        size: "208cm × 265.5cm",
        location: "オルセー美術館",
        description:
          "森の中で着衣の男性と裸体の女性がピクニックをする衝撃的な構図。平坦な色面と明確な輪郭線で、伝統的な明暗法を排した革新的な手法。サロンで大スキャンダルとなった。",
        isAuthentic: true,
        hint: "マネは「近代絵画の父」と呼ばれています。",
        explanation:
          "本物です。マネの問題作にして近代絵画の出発点。裸体の女性が鑑賞者をまっすぐ見つめる大胆さと、伝統的な陰影法を排した平坦な描法が美術界に衝撃を与えました。印象派の先駆者としてのマネの革新性を示す作品です。",
        colors: ["#228B22", "#F5F5DC", "#FFB6C1", "#2F4F4F", "#8B4513"],
        fakeExplanation:
          "構図や人物の表情に微妙な改変があります。マネの平坦な色面と明確な輪郭線による革新的手法が不自然に変更されています。",
        realImage: "images/s4-q9-real.jpg",
        fakeImage: "images/s4-q9-fake.jpg",
      },
    ],
  },

  // ===== Stage 5: ポスト印象派 =====
  {
    id: 5,
    name: "ポスト印象派",
    period: "19世紀末〜20世紀初頭",
    description:
      "印象派の光と色彩の探求を受け継ぎつつ、より個人的な表現や構造的な探求を追求した画家たちの総称。セザンヌは形態の幾何学的分析、ゴッホは激しい感情表現と渦巻く筆致、ゴーギャンは象徴的な色彩と平面的構成、スーラは科学的な点描法をそれぞれ追求しました。「印象派の後」という意味で、統一された様式ではなく、多様な個性の集合です。",
    palette: ["#FF6347", "#4169E1", "#FFD700", "#228B22", "#9370DB"],
    questions: [
      {
        title: "星月夜",
        artist: "フィンセント・ファン・ゴッホ",
        year: "1889年",
        medium: "カンヴァスに油彩",
        size: "73.7cm × 92.1cm",
        location: "ニューヨーク近代美術館",
        description:
          "渦巻く夜空の下、糸杉が立つ村の風景。厚塗りの絵具が波のようにうねり、三日月と星々が強烈な光を放つ。サン＝レミの精神病院の窓から見た景色に想像を加えた作品。",
        isAuthentic: true,
        hint: "ゴッホの独特な筆致と色彩表現に注目。",
        explanation:
          "本物です。ゴッホの最も有名な作品の一つ。精神病院滞在中に制作され、渦巻くような厚塗りの筆触、鮮烈な青と黄の対比はゴッホの内面の激しさを映し出しています。MoMA（ニューヨーク近代美術館）所蔵。",
        colors: ["#191970", "#FFD700", "#4169E1", "#00008B", "#228B22"],
        fakeExplanation:
          "夜空の渦巻きの形や星の配置に微妙な変更があります。ゴッホの厚塗り筆致による激しいうねりが不自然に改変されています。",
        realImage: "images/s5-q1-real.jpg",
        fakeImage: "images/s5-q1-fake.jpg",
      },
      {
        title: "タヒチの女たち",
        artist: "ポール・ゴーギャン",
        year: "1891年",
        medium: "カンヴァスに油彩",
        size: "69cm × 91cm",
        location: "オルセー美術館",
        description:
          "タヒチの浜辺に座る二人の女性。赤い砂浜、青い海、装飾的に平面化された人体が象徴的な色彩で描かれている。西洋的遠近法を放棄し、色面の対比で画面を構成。",
        isAuthentic: true,
        hint: "ゴーギャンが西洋文明から離れて追求したものに注目。",
        explanation:
          "本物です。ゴーギャンがタヒチに渡った最初の年の作品。「原始」の中に人間の本質を求め、象徴的な色彩と平面的な構成で「文明化された」西洋絵画に対抗しました。",
        colors: ["#DC143C", "#FF8C00", "#4682B4", "#228B22", "#8B4513"],
        fakeExplanation:
          "女性の配置や背景の色彩に改変が施されています。ゴーギャンの象徴的な色面構成が微妙に変質しています。",
        realImage: "images/s5-q2-real.jpg",
        fakeImage: "images/s5-q2-fake.jpg",
      },
      {
        title: "サント＝ヴィクトワール山",
        artist: "ポール・セザンヌ",
        year: "1902-1904年頃",
        medium: "カンヴァスに油彩",
        size: "73cm × 91.9cm",
        location: "フィラデルフィア美術館",
        description:
          "プロヴァンスのサント＝ヴィクトワール山を幾何学的な色面の構成で描いた風景画。自然を円筒、球、円錐に還元するという理念が反映され、後のキュビスムを予見する。",
        isAuthentic: true,
        hint: "セザンヌの「自然を幾何学的形態で捉える」理念に注目。",
        explanation:
          "本物です。セザンヌはこの山を生涯にわたって約80回描きました。自然の形態を幾何学的に分析し再構成する手法は、ピカソやブラックのキュビスムに直接的な影響を与え、「近代絵画の父」と呼ばれる所以です。",
        colors: ["#87CEEB", "#228B22", "#DEB887", "#4682B4", "#8B7355"],
        fakeExplanation:
          "山の形態や色面の構成に微妙な違いがあります。セザンヌの幾何学的な自然の分析が不自然に変更されています。",
        realImage: "images/s5-q3-real.jpg",
        fakeImage: "images/s5-q3-fake.jpg",
      },
      {
        title: "グランド・ジャット島の日曜日の午後",
        artist: "ジョルジュ・スーラ",
        year: "1884-1886年",
        medium: "カンヴァスに油彩",
        size: "207.6cm × 308cm",
        location: "シカゴ美術館",
        description:
          "セーヌ川の中州での日曜の憩い。小さな色点を無数に並べる点描法で描かれ、一定の距離から見ると色が網膜上で混合される。科学的な色彩理論に基づいた新印象派の代表作。",
        isAuthentic: true,
        hint: "スーラの科学的アプローチに注目してください。",
        explanation:
          "本物です。スーラが約2年をかけて完成させた記念碑的大作。シュヴルールの色彩対比の法則に基づき、純色の点を並置して視覚的混合を実現する点描法を確立しました。シカゴ美術館の至宝です。",
        colors: ["#228B22", "#87CEEB", "#FFD700", "#FF69B4", "#DDA0DD"],
        fakeExplanation:
          "人物の配置や色点の密度に改変が見られます。スーラの科学的な点描法による色彩混合が不自然に再現されています。",
        realImage: "images/s5-q4-real.jpg",
        fakeImage: "images/s5-q4-fake.jpg",
      },
      {
        title: "プロヴァンスの収穫",
        artist: "フィンセント・ファン・ゴッホ",
        year: "1888年",
        medium: "カンヴァスに油彩",
        size: "72cm × 92cm",
        location: "アムステルダム ゴッホ美術館",
        description:
          "アルルの麦畑の収穫風景。黄金色の麦畑と青い空の対比が鮮烈。素早く力強い筆致で描かれ、南仏の灼熱の太陽の光が画面に溢れている。",
        isAuthentic: true,
        hint: "ゴッホのアルル時代の特徴を確認してください。",
        explanation:
          "本物です。ゴッホがアルル時代に描いた収穫連作の一つ。南仏の強い日差しに触発された黄色と青の鮮烈な対比、厚塗りの力強い筆触はこの時期の特徴です。ゴッホ美術館に所蔵されています。",
        colors: ["#FFD700", "#4169E1", "#DAA520", "#87CEEB", "#FF8C00"],
        fakeExplanation:
          "麦畑の色調や空の表現に微妙な違いがあります。ゴッホのアルル時代特有の鮮烈な黄色と青の対比が変質しています。",
        realImage: "images/s5-q5-real.jpg",
        fakeImage: "images/s5-q5-fake.jpg",
      },
      {
        title: "アルルの星空",
        artist: "ポール・セザンヌ",
        year: "1895年",
        medium: "カンヴァスに油彩",
        size: "68cm × 85cm",
        location: "チューリッヒ個人蔵",
        description:
          "渦巻く夜空と糸杉を描いた夜景画。絵具が厚く盛り上がり、激しいうねりのある筆致で星が光を放っている。強い感情が画面に溢れ出ている。",
        isAuthentic: false,
        hint: "この絵の画風がセザンヌのものかどうか考えてください。",
        explanation:
          "贋作です。「渦巻く夜空」「糸杉」「厚塗り」「激しいうねりのある筆致」「強い感情」はすべてゴッホの特徴であり、セザンヌの画風ではありません。セザンヌは冷静で分析的な形態の探求を特徴とし、感情的な表現は控えました。",
        colors: ["#191970", "#FFD700", "#4169E1", "#00008B", "#228B22"],
        fakeExplanation:
          "描写スタイルがセザンヌではなくゴッホの特徴を示しています。渦巻く筆致と感情的表現はセザンヌの冷静な形態分析と矛盾します。",
        realImage: "images/s5-q6-real.jpg",
        fakeImage: "images/s5-q6-fake.jpg",
      },
      {
        title: "カード遊びをする人々",
        artist: "ポール・セザンヌ",
        year: "1892-1895年頃",
        medium: "カンヴァスに油彩",
        size: "47.5cm × 57cm",
        location: "オルセー美術館",
        description:
          "テーブルを挟んでカード遊びに興じる二人の農夫。簡潔な構図の中に、人物の量感と空間の構造が幾何学的に分析されている。落ち着いた茶系の色調が全体を統一。",
        isAuthentic: true,
        hint: "セザンヌの形態分析のアプローチに注目してください。",
        explanation:
          "本物です。セザンヌが同じ主題で5点描いた連作の一つ。人物と空間を幾何学的に構成し、落ち着いた色調で統一する手法はセザンヌの真骨頂です。2011年にカタール王室が別バージョンを史上最高額で購入したことでも知られます。",
        colors: ["#8B7355", "#A0522D", "#DEB887", "#CD853F", "#556B2F"],
        fakeExplanation:
          "人物のプロポーションやテーブルの構成に微妙な改変があります。セザンヌの幾何学的な人物分析が不自然に変更されています。",
        realImage: "images/s5-q7-real.jpg",
        fakeImage: "images/s5-q7-fake.jpg",
      },
      {
        title: "楽園の門",
        artist: "ポール・ゴーギャン",
        year: "1892年",
        medium: "カンヴァスに油彩",
        size: "88cm × 110cm",
        location: "マルセイユ個人蔵",
        description:
          "タヒチの風景を精密なスーパーリアリズムで描写。写真のように正確な熱帯植物と人物が、厳密な一点透視図法の中に配置されている。光と影は科学的に正確に再現されている。",
        isAuthentic: false,
        hint: "ゴーギャンが追求した表現方法を思い出してください。",
        explanation:
          "贋作です。ゴーギャンは写実を拒否し、象徴的な色彩と平面的な構成を追求した画家です。「スーパーリアリズム」「写真のように正確」「厳密な一点透視図法」はゴーギャンの理念と完全に矛盾します。",
        colors: ["#228B22", "#8B4513", "#87CEEB", "#808080", "#696969"],
        fakeExplanation:
          "描写が写実的すぎ、ゴーギャンの象徴的・平面的な表現とは矛盾する精密さが見られます。",
        realImage: "images/s5-q8-real.jpg",
        fakeImage: "images/s5-q8-fake.jpg",
      },
      {
        title: "自画像（耳を包帯でまいた自画像）",
        artist: "フィンセント・ファン・ゴッホ",
        year: "1889年",
        medium: "カンヴァスに油彩",
        size: "60cm × 49cm",
        location: "コートールド美術館",
        description:
          "耳に包帯を巻いた自画像。青緑色のコートを着た自身を、日本の浮世絵が掛かる部屋の中で描いている。精神的危機の中でも揺るがない画家としての意志が表現されている。",
        isAuthentic: true,
        hint: "ゴッホの有名なエピソードと結びつく作品です。",
        explanation:
          "本物です。ゴッホが自らの耳を切った直後に描いた自画像。背景の浮世絵はゴッホのジャポニスム（日本美術への傾倒）を示しています。精神的危機の中での制作という歴史的事実とも合致する作品です。",
        colors: ["#2F4F4F", "#FF6347", "#F5F5DC", "#4682B4", "#8B7355"],
        fakeExplanation:
          "包帯の位置や背景の浮世絵の描写に微妙な違いがあります。ゴッホの自画像特有の直接的な表現が改変されています。",
        realImage: "images/s5-q9-real.jpg",
        fakeImage: "images/s5-q9-fake.jpg",
      },
    ],
  },

  // ===== Stage 6: 表現主義 =====
  {
    id: 6,
    name: "表現主義",
    period: "20世紀初頭",
    description:
      "20世紀初頭にドイツを中心に展開した芸術運動。外界の客観的な再現ではなく、内面の感情や精神世界の表出を最優先しました。歪んだ形態、激しい色彩、荒々しい筆致で不安、恐怖、孤独、苦悩といった人間の根源的な感情を表現します。ムンク、キルヒナー、カンディンスキー、クレー、シーレらが代表的な画家で、「ブリュッケ」「青騎士」などのグループが結成されました。",
    palette: ["#FF0000", "#000000", "#FFD700", "#4B0082", "#FF4500"],
    questions: [
      {
        title: "叫び",
        artist: "エドヴァルド・ムンク",
        year: "1893年",
        medium: "厚紙にテンペラ・クレヨン",
        size: "91cm × 73.5cm",
        location: "オスロ国立美術館",
        description:
          "橋の上で耳を押さえ口を開ける人物。背景の空が赤く燃え上がり、波打つ線が人物と風景を一体化させる。実存的不安を視覚化した20世紀最も象徴的なイメージの一つ。",
        isAuthentic: true,
        hint: "ムンクの代表的なモチーフと表現手法に注目。",
        explanation:
          "本物です。ムンクの代表作で、複数のバージョンが存在します。「自然を貫く果てしない叫びを感じた」というムンク自身の体験に基づく作品。テンペラとクレヨンという混合技法も記録と一致します。",
        colors: ["#FF4500", "#FF6347", "#FFD700", "#191970", "#2F4F4F"],
        fakeExplanation:
          "人物の表情や背景の波打つ線に微妙な変更があります。ムンクの実存的不安の表現が不自然に改変されています。",
        realImage: "images/s6-q1-real.jpg",
        fakeImage: "images/s6-q1-fake.jpg",
      },
      {
        title: "ベルリンの街路風景",
        artist: "エルンスト・ルートヴィヒ・キルヒナー",
        year: "1913年",
        medium: "カンヴァスに油彩",
        size: "120.6cm × 91.1cm",
        location: "ブリュッケ美術館",
        description:
          "ベルリンの大通りを歩く洗練された女性たち。鋭角的に歪んだ形態と不安定な遠近法、鮮烈な色彩で大都市の喧騒と疎外感を表現。表現主義グループ「ブリュッケ」の代表作。",
        isAuthentic: true,
        hint: "「ブリュッケ」の画家たちの都市描写に注目。",
        explanation:
          "本物です。キルヒナーの代表的な都市風景画。尖った形態、不安定な空間構成、神経質な色彩で近代都市の不安を表現しています。ブリュッケ（橋）派の目指した「新しい表現」を体現する作品です。",
        colors: ["#000000", "#FF69B4", "#4169E1", "#FFD700", "#228B22"],
        fakeExplanation:
          "女性たちの形態や色彩に微妙な違いがあります。キルヒナーの鋭角的で不安定な表現が変質しています。",
        realImage: "images/s6-q2-real.jpg",
        fakeImage: "images/s6-q2-fake.jpg",
      },
      {
        title: "コンポジション VII",
        artist: "ワシリー・カンディンスキー",
        year: "1913年",
        medium: "カンヴァスに油彩",
        size: "200cm × 300cm",
        location: "トレチャコフ美術館",
        description:
          "色彩と形態が渦巻く大型の抽象画。具象的なモチーフが完全に解体され、色と線の純粋なハーモニーが画面を覆う。音楽のような構成を目指した抽象絵画の先駆的作品。",
        isAuthentic: true,
        hint: "カンディンスキーは抽象絵画の開拓者です。",
        explanation:
          "本物です。カンディンスキーが最も複雑で完成度が高いと自負した作品。30以上の習作を経て制作され、色彩と形態による「視覚の音楽」を実現しました。抽象絵画の金字塔です。",
        colors: ["#FF0000", "#4169E1", "#FFD700", "#228B22", "#9370DB"],
        fakeExplanation:
          "色彩の配置や抽象的形態に改変が施されています。カンディンスキーの音楽的な色と線のハーモニーが不自然に変更されています。",
        realImage: "images/s6-q3-real.jpg",
        fakeImage: "images/s6-q3-fake.jpg",
      },
      {
        title: "抱擁",
        artist: "エゴン・シーレ",
        year: "1917年",
        medium: "カンヴァスに油彩",
        size: "100cm × 170.2cm",
        location: "オーストリア・ギャラリー",
        description:
          "抱き合う二人の人物。歪んだ身体の線、露わな肉体、激しい色彩で人間の官能と苦悩を同時に表現。シーレ特有の鋭い輪郭線と肌の表現が際立つ。",
        isAuthentic: true,
        hint: "シーレの人体表現の特徴を確認してください。",
        explanation:
          "本物です。シーレの代表作の一つ。歪んだ身体表現、鋭い輪郭線、露わで生々しい肉体描写はシーレの独自様式です。師であるクリムトの装飾性を脱却し、より直接的な感情表現を追求しました。",
        colors: ["#DEB887", "#8B4513", "#F5F5DC", "#FF6347", "#2F4F4F"],
        fakeExplanation:
          "人物の輪郭線や肉体表現に微妙な変更があります。シーレ特有の歪んだ身体表現と鋭い線描が改変されています。",
        realImage: "images/s6-q4-real.jpg",
        fakeImage: "images/s6-q4-fake.jpg",
      },
      {
        title: "青い馬の塔",
        artist: "フランツ・マルク",
        year: "1913年",
        medium: "カンヴァスに油彩",
        size: "200cm × 130cm",
        location: "行方不明（戦時中に消失）",
        description:
          "重なり合うように立つ青い馬たちの記念碑的構図。有機的な曲線と幾何学的形態が融合し、青を基調とした大胆な色彩で動物の精神性を表現している。",
        isAuthentic: true,
        hint: "「青騎士」グループの画家たちは色に精神的意味を込めました。",
        explanation:
          "本物です。マルクの代表作。「青騎士」グループの理念を体現し、青は「精神性」を、馬は「純粋さ」を象徴しています。第二次大戦中に消失し、現在は写真でのみ知ることができます。",
        colors: ["#4169E1", "#00008B", "#87CEEB", "#228B22", "#FFD700"],
        fakeExplanation:
          "馬の青色の色調や構図のバランスに微妙な違いがあります。マルクの精神性を象徴する色彩表現が変質しています。",
        realImage: "images/s6-q5-real.jpg",
        fakeImage: "images/s6-q5-fake.jpg",
      },
      {
        title: "ミュンヘンの静物",
        artist: "ワシリー・カンディンスキー",
        year: "1910年",
        medium: "カンヴァスに油彩",
        size: "85cm × 110cm",
        location: "ミュンヘン個人蔵",
        description:
          "テーブルの上の果物と花瓶を、17世紀オランダ絵画のように精密に写実的に描いた静物画。光と影の正確な再現と、細部にわたる質感描写が特徴。古典的な三角構図が安定感を与えている。",
        isAuthentic: false,
        hint: "1910年のカンディンスキーの画風を考えてみてください。",
        explanation:
          "贋作です。1910年のカンディンスキーはすでに抽象への道を歩み始めており、「17世紀オランダ絵画のように精密に写実的」な描法に戻ることはありえません。同年、彼は最初の抽象水彩画を制作しています。",
        colors: ["#8B7355", "#556B2F", "#CD853F", "#DEB887", "#A0522D"],
        fakeExplanation:
          "描写スタイルが抽象に向かうカンディンスキーの1910年の画風と矛盾する写実的要素が含まれています。",
        realImage: "images/s6-q6-real.jpg",
        fakeImage: "images/s6-q6-fake.jpg",
      },
      {
        title: "月夜の浜辺",
        artist: "エドヴァルド・ムンク",
        year: "1895年",
        medium: "カンヴァスに油彩",
        size: "93cm × 110cm",
        location: "オスロ ムンク美術館",
        description:
          "月明かりに照らされた浜辺に佇む孤独な人物。水面に映る月光が長く伸び、青と白の冷たい色調が静寂と孤独を強調する。人物の影が不穏に歪んでいる。",
        isAuthentic: true,
        hint: "ムンクの繰り返し描いたテーマに注目。",
        explanation:
          "本物です。ムンクは「生命のフリーズ」と呼ぶ連作の中で、孤独、不安、愛、死をテーマに繰り返し描きました。月夜の浜辺は彼が好んだモチーフの一つで、青い色調による孤独の表現はムンクの真骨頂です。",
        colors: ["#191970", "#4682B4", "#C0C0C0", "#000080", "#F5F5DC"],
        fakeExplanation:
          "月光の反射や人物の影に微妙な変更があります。ムンクの青い色調による孤独の表現が不自然に改変されています。",
        realImage: "images/s6-q7-real.jpg",
        fakeImage: "images/s6-q7-fake.jpg",
      },
      {
        title: "春の目覚め",
        artist: "エゴン・シーレ",
        year: "1916年",
        medium: "カンヴァスに油彩",
        size: "75cm × 100cm",
        location: "ウィーン個人蔵",
        description:
          "花咲く庭園を水彩画のような淡い色調で描いた風景画。人物は一切登場せず、穏やかで牧歌的な雰囲気に満ちている。柔らかな輪郭線と繊細なグラデーションが心地よい。",
        isAuthentic: false,
        hint: "シーレの画風の核心にあるものは何でしょう？",
        explanation:
          "贋作です。シーレは歪んだ人体、鋭い輪郭線、生々しい感情表現を特徴とする画家です。「淡い色調」「穏やかで牧歌的」「柔らかな輪郭線」はシーレの激しく内省的な画風と正反対です。また、彼が人物なしの純粋な風景画を描くことは稀です。",
        colors: ["#90EE90", "#FFB6C1", "#87CEEB", "#FFDAB9", "#E6E6FA"],
        fakeExplanation:
          "色調や構図がシーレの激しい表現主義的画風とは矛盾する穏やかさを示しています。",
        realImage: "images/s6-q8-real.jpg",
        fakeImage: "images/s6-q8-fake.jpg",
      },
      {
        title: "ゲルニカの習作",
        artist: "フランツ・マルク",
        year: "1914年",
        medium: "カンヴァスに油彩",
        size: "95cm × 145cm",
        location: "ベルリン個人蔵",
        description:
          "戦争の恐怖をモノクロームで描いた大作。引き裂かれた人体と動物が画面を埋め尽くし、キュビスム的な断片化で暴力の混沌を表現している。",
        isAuthentic: false,
        hint: "作品のタイトルと歴史的事実を照合してみてください。",
        explanation:
          "贋作です。「ゲルニカ」はスペイン内戦中の1937年の爆撃事件を題材にピカソが描いた作品です。マルクは1916年に第一次大戦で戦死しており、1937年の事件の「習作」を描くことは時系列的に不可能です。",
        colors: ["#000000", "#808080", "#C0C0C0", "#FFFFFF", "#696969"],
        fakeExplanation:
          "歴史的に不可能な要素が含まれています。マルクの没年と「ゲルニカ」の歴史的背景が矛盾します。",
        realImage: "images/s6-q9-real.jpg",
        fakeImage: "images/s6-q9-fake.jpg",
      },
    ],
  },

  // ===== Stage 7: シュルレアリスム =====
  {
    id: 7,
    name: "シュルレアリスム",
    period: "1920〜1960年代",
    description:
      "フロイトの精神分析に影響を受け、夢、無意識、自動記述を創作の源泉とした芸術運動。1924年にアンドレ・ブルトンが「シュルレアリスム宣言」を発表し始まりました。ダリの写実的に描かれた夢幻世界、マグリットの知的なイメージの転覆、ミロの有機的な抽象形態、エルンストのフロッタージュなど、多様な手法で「超現実」を探求しました。",
    palette: ["#4B0082", "#FF4500", "#00CED1", "#FFD700", "#DC143C"],
    questions: [
      {
        title: "記憶の固執",
        artist: "サルバドール・ダリ",
        year: "1931年",
        medium: "カンヴァスに油彩",
        size: "24.1cm × 33cm",
        location: "ニューヨーク近代美術館",
        description:
          "溶けて垂れ下がる時計が荒涼とした風景に置かれた小品。超写実的な描写力で不条理な夢の光景を描き出す。柔らかい時計は時間の主観的体験を象徴している。",
        isAuthentic: true,
        hint: "ダリの「偏執狂的批判的方法」に注目。",
        explanation:
          "本物です。ダリの最も有名な作品。「柔らかい時計」のモチーフはカマンベールチーズが溶ける様子から着想を得たとダリ自身が語っています。小さな画面に驚異的な精密さで夢の世界を描き出す技法はダリの真骨頂です。",
        colors: ["#DEB887", "#87CEEB", "#8B7355", "#CD853F", "#4682B4"],
        fakeExplanation:
          "溶けた時計の形状や砂漠の色調に微妙な変更があります。ダリの超写実的な描写力による夢の表現が改変されています。",
        realImage: "images/s7-q1-real.jpg",
        fakeImage: "images/s7-q1-fake.jpg",
      },
      {
        title: "イメージの裏切り",
        artist: "ルネ・マグリット",
        year: "1929年",
        medium: "カンヴァスに油彩",
        size: "63.5cm × 93.98cm",
        location: "ロサンゼルス・カウンティ美術館",
        description:
          "パイプの絵の下に「これはパイプではない」と書かれた作品。描かれた対象と実物の関係、イメージと言語の乖離を問いかける知的な作品。",
        isAuthentic: true,
        hint: "マグリットの哲学的なアプローチに注目。",
        explanation:
          "本物です。マグリットの代表作。「絵に描かれたパイプは実物のパイプではない」という当たり前の事実を提示することで、表象と現実の関係を根本から問い直しました。言語哲学者フーコーも論考を捧げた作品です。",
        colors: ["#DEB887", "#8B4513", "#F5F5DC", "#2F4F4F", "#A0522D"],
        fakeExplanation:
          "パイプの形やテキストの配置に微妙な違いがあります。マグリットの知的で正確なイメージ構成が不自然に変更されています。",
        realImage: "images/s7-q2-real.jpg",
        fakeImage: "images/s7-q2-fake.jpg",
      },
      {
        title: "カタルーニャの風景",
        artist: "ジョアン・ミロ",
        year: "1923-1924年",
        medium: "カンヴァスに油彩",
        size: "64.8cm × 100.3cm",
        location: "ニューヨーク近代美術館",
        description:
          "黄土色の大地にアメーバのような有機的形態が浮遊する。記号的な目、耳、動物、植物が夢の中のように散りばめられ、カタルーニャの農村風景が幻想的に変容している。",
        isAuthentic: true,
        hint: "ミロの有機的形態と記号的表現に注目。",
        explanation:
          "本物です。ミロがシュルレアリスムに接近した重要な転換点の作品。現実の風景を有機的な記号に解体し再構成する手法は、以後のミロの独自様式を確立しました。MoMA所蔵。",
        colors: ["#DAA520", "#FFD700", "#FF0000", "#228B22", "#000000"],
        fakeExplanation:
          "有機的形態の配置や色彩に改変が施されています。ミロの夢幻的な記号世界が微妙に変質しています。",
        realImage: "images/s7-q3-real.jpg",
        fakeImage: "images/s7-q3-fake.jpg",
      },
      {
        title: "人の子",
        artist: "ルネ・マグリット",
        year: "1964年",
        medium: "カンヴァスに油彩",
        size: "116cm × 89cm",
        location: "個人蔵",
        description:
          "スーツにボウラーハットの男性の顔が青りんごで隠されている自画像的作品。日常的なイメージの組み合わせが不思議な違和感を生み出す。海と曇り空の前に佇む構図。",
        isAuthentic: true,
        hint: "マグリットのボウラーハットの男のモチーフに注目。",
        explanation:
          "本物です。マグリットが繰り返し描いたボウラーハットの男のモチーフの代表作。「見えるものはいつも別のものを隠している」というマグリットの哲学を体現。自画像であると同時に、すべての人間の匿名性を象徴しています。",
        colors: ["#87CEEB", "#228B22", "#2F4F4F", "#696969", "#808080"],
        fakeExplanation:
          "りんごの位置やスーツの色調に微妙な違いがあります。マグリットの日常的イメージの組み合わせによる違和感が変更されています。",
        realImage: "images/s7-q4-real.jpg",
        fakeImage: "images/s7-q4-fake.jpg",
      },
      {
        title: "夢の解剖学",
        artist: "サルバドール・ダリ",
        year: "1935年",
        medium: "木板にアクリル",
        size: "42cm × 56cm",
        location: "バルセロナ個人蔵",
        description:
          "夢の中の解剖学的形態を超写実的に描いた作品。溶けた人体のパーツが砂漠に散らばり、引き出し付きの人体が中央に鎮座している。鮮やかなアクリル絵具の発色が目を引く。",
        isAuthentic: false,
        hint: "1935年に使用可能だった画材を確認してください。",
        explanation:
          "贋作です。アクリル絵具が画材として市販されたのは1950年代以降です。1935年にダリがアクリルを使用することは不可能でした。ダリは油彩を主に使用しており、画材の時代考証が合いません。",
        colors: ["#DEB887", "#FF4500", "#FFD700", "#87CEEB", "#DC143C"],
        fakeExplanation:
          "画材や色の発色にダリの時代には不可能な特徴が見られます。アクリル絵具の使用は時代考証的に矛盾します。",
        realImage: "images/s7-q5-real.jpg",
        fakeImage: "images/s7-q5-fake.jpg",
      },
      {
        title: "透視（大家族）",
        artist: "ルネ・マグリット",
        year: "1953年",
        medium: "カンヴァスに油彩",
        size: "100cm × 81cm",
        location: "個人蔵",
        description:
          "曇り空を背景に、鳥のシルエットの中に青空と白い雲が見える。鳥の外側は灰色の海と空、内側は晴れた空という反転の構図が、見ることの不思議さを問う。",
        isAuthentic: true,
        hint: "マグリットのイメージの反転・入れ替え手法に注目。",
        explanation:
          "本物です。マグリットの代表的手法である「内と外の反転」が見事に実現された作品。鳥の形をした「窓」から別の現実が見えるという発想は、マグリットの知的なシュルレアリスムの真髄です。",
        colors: ["#808080", "#87CEEB", "#FFFFFF", "#4682B4", "#A9A9A9"],
        fakeExplanation:
          "鳥のシルエットや内外の空の色調に微妙な改変があります。マグリットの反転手法の精密さが損なわれています。",
        realImage: "images/s7-q6-real.jpg",
        fakeImage: "images/s7-q6-fake.jpg",
      },
      {
        title: "セレベスの象",
        artist: "マックス・エルンスト",
        year: "1921年",
        medium: "カンヴァスに油彩",
        size: "125.4cm × 107.9cm",
        location: "テート・モダン",
        description:
          "巨大な機械的な象のような形態が画面中央にそびえる。コラージュ的な要素が組み合わされ、工業製品と有機体が融合した不可思議な存在が描かれている。",
        isAuthentic: true,
        hint: "エルンストの機械的形態とコラージュ的手法に注目。",
        explanation:
          "本物です。エルンストの代表作。アフリカの穀物貯蔵庫の写真から着想を得た機械的な象は、エルンストのコラージュ的発想を絵画に転用した初期シュルレアリスムの傑作です。テート・モダン所蔵。",
        colors: ["#808080", "#556B2F", "#FF6347", "#2F4F4F", "#DAA520"],
        fakeExplanation:
          "機械的な象の形態やコラージュ的要素に変更が加えられています。エルンストの独自の造形感覚が改変されています。",
        realImage: "images/s7-q7-real.jpg",
        fakeImage: "images/s7-q7-fake.jpg",
      },
      {
        title: "無意識の庭園",
        artist: "ジョアン・ミロ",
        year: "1928年",
        medium: "カンヴァスに油彩",
        size: "92cm × 72cm",
        location: "ジュネーブ個人蔵",
        description:
          "完全な写実画法で描かれたバルセロナの植物園。植物の一本一本が植物図鑑のように精密に描かれ、光と影が科学的に正確に再現されている。遠近法も厳密に適用されている。",
        isAuthentic: false,
        hint: "タイトルの「無意識」とこの絵の描写方法に矛盾はないですか？",
        explanation:
          "贋作です。ミロは「絵画の暗殺」を宣言し、従来の写実を徹底的に拒否した画家です。「植物図鑑のように精密」「科学的に正確」「厳密な遠近法」はミロの有機的で自由な表現とは完全に矛盾します。",
        colors: ["#228B22", "#556B2F", "#8B7355", "#90EE90", "#DEB887"],
        fakeExplanation:
          "描写がミロの有機的で自由な表現とは矛盾する写実的精密さを示しています。",
        realImage: "images/s7-q8-real.jpg",
        fakeImage: "images/s7-q8-fake.jpg",
      },
      {
        title: "燃えるキリン",
        artist: "サルバドール・ダリ",
        year: "1937年",
        medium: "カンヴァスに油彩",
        size: "35cm × 27cm",
        location: "バーゼル市立美術館",
        description:
          "背景で燃えるキリンと、引き出しが飛び出した女性像が描かれた小品。松葉杖で支えられた柔らかい形態が超写実的に描かれ、戦争の予兆とフロイト的象徴が共存する。",
        isAuthentic: true,
        hint: "ダリの「引き出し」のモチーフはフロイトの影響です。",
        explanation:
          "本物です。ダリがスペイン内戦を予感して描いた作品。「引き出し付きの人体」はフロイトの無意識理論の視覚化であり、「燃えるキリン」は戦争の恐怖を象徴しています。ダリの偏執狂的批判的方法の典型です。",
        colors: ["#FF4500", "#FFD700", "#87CEEB", "#DEB887", "#DC143C"],
        fakeExplanation:
          "燃えるキリンや引き出し付き人体の表現に微妙な変更があります。ダリのフロイト的象徴が改変されています。",
        realImage: "images/s7-q9-real.jpg",
        fakeImage: "images/s7-q9-fake.jpg",
      },
    ],
  },
  // ===== Stage 8: 浮世絵 =====
  {
    id: 8,
    name: "浮世絵",
    period: "17〜19世紀",
    description:
      "江戸時代に花開いた大衆芸術。「浮き世」すなわち現世の享楽を描き、木版多色摺（錦絵）の技法により大量生産が可能となりました。美人画、役者絵、風景画、武者絵など多彩なジャンルが発展。喜多川歌麿の美人画、東洲斎写楽の役者大首絵、葛飾北斎と歌川広重の風景画が代表的です。19世紀にはヨーロッパに渡り、印象派の画家たちに大きな影響（ジャポニスム）を与えました。",
    palette: ["#1B3A5C", "#C41E3A", "#F0E68C", "#2F5F2F", "#F5F5DC"],
    questions: [
      {
        title: "冨嶽三十六景 神奈川沖浪裏",
        artist: "葛飾北斎",
        year: "1831年頃",
        medium: "木版多色摺（錦絵）",
        size: "25.7cm × 37.9cm",
        location: "メトロポリタン美術館ほか多数",
        description:
          "巨大な波が小舟を呑み込もうとする瞬間を捉えた構図。波頭の飛沫が鉤爪のように描かれ、遠景に富士山が小さく静かに佇む。大胆な構図と藍色（ベロ藍）の使用が革新的。",
        isAuthentic: true,
        hint: "北斎の代表的なシリーズ名と、使用されている顔料に注目。",
        explanation:
          "本物です。北斎70歳頃の代表作で、冨嶽三十六景シリーズの一枚。当時輸入された化学顔料「ベロ藍（プルシアンブルー）」を効果的に使用し、西洋の遠近法と日本の伝統的表現を融合させました。世界で最も有名な日本美術作品の一つです。",
        colors: ["#1B3A5C", "#4682B4", "#F5F5DC", "#2F5F2F", "#87CEEB"],
        fakeExplanation:
          "波の形状や富士山の位置に微妙な違いがあります。北斎のベロ藍による大胆な波の表現が不自然に改変されています。",
        realImage: "images/s8-q1-real.jpg",
        fakeImage: "images/s8-q1-fake.jpg",
      },
      {
        title: "東海道五十三次 日本橋 朝之景",
        artist: "歌川広重",
        year: "1833-1834年",
        medium: "木版多色摺（錦絵）",
        size: "約25cm × 37cm",
        location: "東京国立博物館ほか",
        description:
          "大名行列が日本橋を渡る早朝の光景。朝靄に包まれた橋と人々の活気が叙情的に描かれ、広重特有の湿潤な空気感が画面全体を包み込んでいる。",
        isAuthentic: true,
        hint: "広重の代表的なシリーズと、その叙情的表現に注目。",
        explanation:
          "本物です。広重の出世作「東海道五十三次」の起点となる一枚。空気や湿度を感じさせる叙情的な風景表現は広重の真骨頂であり、ゴッホやモネにも影響を与えました。",
        colors: ["#F0E68C", "#FFB6C1", "#87CEEB", "#DEB887", "#2F5F2F"],
        fakeExplanation:
          "朝靄の表現や人物の配置に微妙な変更があります。広重の叙情的な空気感の描写が改変されています。",
        realImage: "images/s8-q2-real.jpg",
        fakeImage: "images/s8-q2-fake.jpg",
      },
      {
        title: "ビードロを吹く娘",
        artist: "喜多川歌麿",
        year: "1790年頃",
        medium: "木版多色摺（錦絵）",
        size: "約36cm × 25cm",
        location: "東京国立博物館",
        description:
          "ガラス玩具のビードロを吹く若い女性の上半身を描いた美人画。透き通るビードロと女性の繊細な表情が、雲母摺の銀色の背景に映える。歌麿の大首絵の代表作。",
        isAuthentic: true,
        hint: "歌麿の美人画の特徴と、背景の摺り技法に注目。",
        explanation:
          "本物です。歌麿の代表作。「大首絵」と呼ばれる胸から上のクローズアップ構図と、雲母（きら）摺による銀色の背景は歌麿の革新的な手法です。女性の内面の感情を繊細に描き出す歌麿の真骨頂。",
        colors: ["#C0C0C0", "#FFB6C1", "#F5F5DC", "#DDA0DD", "#E6E6FA"],
        fakeExplanation:
          "ビードロや女性の表情に微妙な違いがあります。歌麿の繊細な美人画の表現が不自然に変更されています。",
        realImage: "images/s8-q3-real.jpg",
        fakeImage: "images/s8-q3-fake.jpg",
      },
      {
        title: "三代目大谷鬼次の奴江戸兵衛",
        artist: "東洲斎写楽",
        year: "1794年",
        medium: "木版多色摺（錦絵）",
        size: "約38cm × 25cm",
        location: "東京国立博物館",
        description:
          "歌舞伎役者の大胆にデフォルメされた表情を捉えた大首絵。両手を広げた誇張されたポーズと、鋭い目つきが役柄の悪人の性格を見事に表現。暗雲母摺の背景。",
        isAuthentic: true,
        hint: "写楽の活動期間と作品の特徴に注目。",
        explanation:
          "本物です。写楽の最も有名な作品。わずか10ヶ月ほどの活動期間に約145点の作品を残した謎の絵師。大胆なデフォルメと心理描写は、当時の役者絵の常識を覆す革新的表現でした。",
        colors: ["#696969", "#DEB887", "#C41E3A", "#F5F5DC", "#2C1810"],
        fakeExplanation:
          "役者の表情や衣装のデフォルメに微妙な改変があります。写楽の鋭い心理描写が変質しています。",
        realImage: "images/s8-q4-real.jpg",
        fakeImage: "images/s8-q4-fake.jpg",
      },
      {
        title: "冨嶽三十六景 凱風快晴",
        artist: "葛飾北斎",
        year: "1831年頃",
        medium: "木版多色摺（錦絵）",
        size: "25.7cm × 37.9cm",
        location: "東京国立博物館ほか",
        description:
          "朝焼けに赤く染まる富士山。裾野の緑の樹海と青い空に白い雲が浮かぶシンプルな構図ながら、富士の圧倒的な存在感を表現。通称「赤富士」。",
        isAuthentic: true,
        hint: "北斎の冨嶽三十六景シリーズの中でも特に有名な一枚。",
        explanation:
          "本物です。「赤富士」の通称で親しまれる北斎の代表作。朝焼けの一瞬を捉えたシンプルな構図の中に、富士の荘厳さと自然の壮大さが凝縮されています。冨嶽三十六景シリーズの一枚。",
        colors: ["#C41E3A", "#228B22", "#87CEEB", "#F5F5DC", "#8B4513"],
        fakeExplanation:
          "富士山の赤色の色調や裾野の緑に微妙な違いがあります。北斎の朝焼け表現の繊細さが改変されています。",
        realImage: "images/s8-q5-real.jpg",
        fakeImage: "images/s8-q5-fake.jpg",
      },
      {
        title: "名所江戸百景 亀戸梅屋舗",
        artist: "歌川広重",
        year: "1857年",
        medium: "木版多色摺（錦絵）",
        size: "約36cm × 24cm",
        location: "ブルックリン美術館ほか",
        description:
          "手前に大胆に配置された梅の枝が画面を横切り、奥に梅園を楽しむ人々が小さく描かれる。前景と後景の極端な対比と、赤い空の大胆な色使いが印象的。",
        isAuthentic: true,
        hint: "この構図がある印象派の画家に模写されたことで有名。",
        explanation:
          "本物です。広重晩年の傑作シリーズ「名所江戸百景」の一枚。大胆な手前の梅の枝の構図はゴッホが油彩で模写したことでも知られます。日本美術のヨーロッパへの影響（ジャポニスム）を示す象徴的な作品です。",
        colors: ["#C41E3A", "#8B4513", "#FF6347", "#228B22", "#F0E68C"],
        fakeExplanation:
          "梅の枝の配置や背景の赤い空の色調に微妙な変更があります。広重の大胆な構図と色使いが不自然に変更されています。",
        realImage: "images/s8-q6-real.jpg",
        fakeImage: "images/s8-q6-fake.jpg",
      },
      {
        title: "江戸名所 品川の夕映え",
        artist: "葛飾北斎",
        year: "1825年",
        medium: "カンヴァスに油彩",
        size: "89cm × 130cm",
        location: "ロンドン個人蔵",
        description:
          "品川沖の夕暮れを西洋画の技法で描いた風景画。カンヴァスに油彩絵具を用い、遠近法と陰影法を駆使して立体的に描かれている。厚塗りの絵具の質感が豊かな表現を生んでいる。",
        isAuthentic: false,
        hint: "浮世絵師が使用する画材と技法を確認してください。",
        explanation:
          "贋作です。北斎は木版画と肉筆画（紙や絹に岩絵具や墨で描く）を主な手法としており、カンヴァスに油彩で描くことはありません。江戸時代の日本では油彩画の画材は一般に流通しておらず、浮世絵師が油彩を使用することは時代考証的に不可能です。",
        colors: ["#FF8C00", "#4682B4", "#FFD700", "#8B4513", "#2F4F4F"],
        fakeExplanation:
          "西洋画の画材と技法で描かれており、浮世絵師の伝統的な木版画や肉筆画の技法とは根本的に異なります。",
        realImage: "images/s8-q7-real.jpg",
        fakeImage: "images/s8-q7-fake.jpg",
      },
      {
        title: "吉原遊廓 花魁道中図",
        artist: "東洲斎写楽",
        year: "1795年",
        medium: "木版多色摺（錦絵）",
        size: "約38cm × 25cm",
        location: "大阪個人蔵",
        description:
          "華やかな衣装をまとった花魁が遊廓を練り歩く様子を描いた美人画。繊細な着物の文様と優美なポーズが、写楽の柔らかな筆致で丁寧に描かれている。",
        isAuthentic: false,
        hint: "写楽が描いた主題と作風を思い出してください。",
        explanation:
          "贋作です。写楽は歌舞伎役者の大首絵を専門とした絵師であり、美人画や遊廓の風俗画は描いていません。また写楽の画風は「大胆なデフォルメ」と「鋭い心理描写」が特徴で、「繊細」「柔らかな筆致」「優美」という表現は写楽の作風と矛盾します。",
        colors: ["#FFB6C1", "#FFD700", "#C41E3A", "#E6E6FA", "#DDA0DD"],
        fakeExplanation:
          "描かれている主題が写楽の専門である歌舞伎役者絵ではなく美人画であり、画風も写楽の大胆なデフォルメとは矛盾します。",
        realImage: "images/s8-q8-real.jpg",
        fakeImage: "images/s8-q8-fake.jpg",
      },
      {
        title: "東海道 蒸気車之図",
        artist: "歌川広重",
        year: "1850年",
        medium: "木版多色摺（錦絵）",
        size: "約25cm × 37cm",
        location: "横浜個人蔵",
        description:
          "東海道を走る蒸気機関車と富士山を組み合わせた風景画。煙を吐く鉄の機関車が広重特有の叙情的な風景の中を走り抜ける近代化の象徴的場面。",
        isAuthentic: false,
        hint: "日本の鉄道の歴史と広重の没年を確認してください。",
        explanation:
          "贋作です。日本初の鉄道が新橋〜横浜間に開通したのは1872年（明治5年）です。広重は1858年に亡くなっており、鉄道を目にすることは不可能でした。1850年の日本にはまだ蒸気機関車は存在しません。",
        colors: ["#696969", "#808080", "#2F4F4F", "#1B3A5C", "#A9A9A9"],
        fakeExplanation:
          "描かれている蒸気機関車は広重の没後に日本に導入されたものであり、時代考証的に不可能な内容です。",
        realImage: "images/s8-q9-real.jpg",
        fakeImage: "images/s8-q9-fake.jpg",
      },
    ],
  },

  // ===== Stage 9: 琳派 =====
  {
    id: 9,
    name: "琳派",
    period: "17〜19世紀",
    description:
      "17世紀の京都で俵屋宗達が創始し、約100年後に尾形光琳が大成、さらに100年後に酒井抱一・鈴木其一が江戸で継承した装飾芸術の系譜。師弟関係ではなく、時を超えた私淑（作品を通じた学び）により伝承された世界的にも稀有な芸術流派です。金銀地に大胆な構図、たらし込み技法、自然の意匠化、装飾性と叙情性の融合が特徴です。",
    palette: ["#DAA520", "#00008B", "#228B22", "#C0C0C0", "#8B0000"],
    questions: [
      {
        title: "風神雷神図屏風",
        artist: "俵屋宗達",
        year: "17世紀前半",
        medium: "紙本金地着色 二曲一双",
        size: "各154.5cm × 169.8cm",
        location: "建仁寺（京都国立博物館寄託）",
        description:
          "金地の空間に風神と雷神が躍動する二曲一双の屏風。余白を大胆に活かした構図で、風袋を広げる風神と太鼓を叩く雷神が画面の端に配され、中央の金色の空間に無限の広がりを生む。",
        isAuthentic: true,
        hint: "琳派の始祖とされる画家の代表的な構図に注目。",
        explanation:
          "本物です。琳派の始祖・宗達の最高傑作で国宝。金地の余白を「間」として活用する大胆な構図は、後に光琳、抱一がそれぞれ模写するほどの影響力を持ちました。建仁寺所蔵（京都国立博物館寄託）。",
        colors: ["#DAA520", "#228B22", "#808080", "#F5F5DC", "#4682B4"],
        fakeExplanation:
          "風神雷神の配置や金地の質感に微妙な違いがあります。宗達の大胆な余白の使い方が不自然に改変されています。",
        realImage: "images/s9-q1-real.jpg",
        fakeImage: "images/s9-q1-fake.jpg",
      },
      {
        title: "燕子花図屏風",
        artist: "尾形光琳",
        year: "1701-1705年頃",
        medium: "紙本金地着色 六曲一双",
        size: "各150.9cm × 338.8cm",
        location: "根津美術館",
        description:
          "金地に群青と緑青のみで燕子花（かきつばた）の群落を描いた屏風。リズミカルに反復する花と葉の配置が音楽的な律動を生み、装飾性と自然の生命力が融合している。",
        isAuthentic: true,
        hint: "光琳の大胆な色彩の限定と、パターンの反復に注目。",
        explanation:
          "本物です。光琳の代表作で国宝。群青と緑青の二色のみで燕子花の群落を表現した大胆さ、パターンの反復によるリズム感は琳派の装飾美の極致です。『伊勢物語』八橋の段に着想を得た作品。根津美術館所蔵。",
        colors: ["#DAA520", "#00008B", "#228B22", "#4169E1", "#2F5F2F"],
        fakeExplanation:
          "燕子花の群青と緑青の色調やリズミカルなパターンに微妙な変更があります。光琳の装飾美の精緻さが損なわれています。",
        realImage: "images/s9-q2-real.jpg",
        fakeImage: "images/s9-q2-fake.jpg",
      },
      {
        title: "紅白梅図屏風",
        artist: "尾形光琳",
        year: "18世紀初頭",
        medium: "紙本金銀地着色 二曲一双",
        size: "各156cm × 172.2cm",
        location: "MOA美術館",
        description:
          "中央を流れる装飾的な水流を挟み、左に白梅、右に紅梅が対峙する構図。金銀の地に「たらし込み」技法で描かれた幹の豊かな表情と、図案化された水流の曲線美が際立つ。",
        isAuthentic: true,
        hint: "光琳の「たらし込み」技法と装飾的構成に注目。",
        explanation:
          "本物です。光琳晩年の最高傑作で国宝。墨の濃淡を滲ませる「たらし込み」技法による樹幹の表現、銀地に描かれた水流の図案的な曲線美は琳派の技法の集大成です。MOA美術館所蔵。",
        colors: ["#DAA520", "#C0C0C0", "#8B0000", "#F5F5DC", "#556B2F"],
        fakeExplanation:
          "梅の幹のたらし込み表現や水流の曲線に微妙な違いがあります。光琳の金銀地の技法が不自然に変更されています。",
        realImage: "images/s9-q3-real.jpg",
        fakeImage: "images/s9-q3-fake.jpg",
      },
      {
        title: "夏秋草図屏風",
        artist: "酒井抱一",
        year: "1821年頃",
        medium: "紙本銀地着色 二曲一双",
        size: "各164.5cm × 181.8cm",
        location: "東京国立博物館",
        description:
          "銀地に夏の夕立に打たれる百合や向日葵、秋の野分に揺れる薄や葛を描いた屏風。光琳の「風神雷神図」の裏面に描かれた作品で、風雨に耐える草花の繊細な叙情性が特徴。",
        isAuthentic: true,
        hint: "抱一が光琳へのオマージュとして制作した関係に注目。",
        explanation:
          "本物です。抱一の代表作で重要文化財。光琳の「風神雷神図屏風」（抱一が模写した版）の裏面に描かれ、風神→夕立の夏草、雷神→野分の秋草という見事な対応関係を持ちます。江戸琳派の叙情性の極致。",
        colors: ["#C0C0C0", "#228B22", "#DAA520", "#DDA0DD", "#8B7355"],
        fakeExplanation:
          "草花の配置や銀地の質感に微妙な改変があります。抱一の繊細な叙情性の表現が変質しています。",
        realImage: "images/s9-q4-real.jpg",
        fakeImage: "images/s9-q4-fake.jpg",
      },
      {
        title: "朝顔図屏風",
        artist: "鈴木其一",
        year: "19世紀前半",
        medium: "紙本金地着色 六曲一双",
        size: "各178.2cm × 379.8cm",
        location: "メトロポリタン美術館",
        description:
          "金地に大胆に配された朝顔の群青の花と緑の蔓。光琳の燕子花図を意識しながらも、より鋭角的で現代的なデザイン感覚で植物の生命力を描き出している。",
        isAuthentic: true,
        hint: "其一の師・抱一との関係と、光琳への系譜に注目。",
        explanation:
          "本物です。其一の代表作。光琳の「燕子花図屏風」を意識しながら、より大胆でシャープな造形感覚を示しています。金地に群青の花が映える配色は琳派の伝統を継承しつつ、其一独自の近代的感性が光る傑作。メトロポリタン美術館所蔵。",
        colors: ["#DAA520", "#00008B", "#228B22", "#4169E1", "#F5F5DC"],
        fakeExplanation:
          "朝顔の群青の花や蔓の配置に微妙な違いがあります。其一のシャープな造形感覚が改変されています。",
        realImage: "images/s9-q5-real.jpg",
        fakeImage: "images/s9-q5-fake.jpg",
      },
      {
        title: "洛中洛外の四季",
        artist: "俵屋宗達",
        year: "1625年頃",
        medium: "カンヴァスに油彩",
        size: "各180cm × 360cm",
        location: "ロンドン個人蔵",
        description:
          "京都の四季を西洋画法で描いた大型風景画。カンヴァスに油彩で精密な陰影法と一点透視図法を駆使し、建物や人物が写実的に描かれている。西洋絵画の手法が全面的に採用されている。",
        isAuthentic: false,
        hint: "17世紀初頭の日本の画家が使用できた画材を考えてください。",
        explanation:
          "贋作です。宗達は紙本・絹本に岩絵具や墨で描いた画家です。17世紀初頭の日本ではカンヴァスに油彩という西洋画材は一般的に入手不可能でした。また宗達は装飾的で大胆な構図を特徴とし、「写実的」「一点透視図法」は宗達の画風と根本的に異なります。",
        colors: ["#8B7355", "#556B2F", "#CD853F", "#696969", "#A0522D"],
        fakeExplanation:
          "西洋画の画材と技法で描かれており、17世紀初頭の日本の画家の技法とは根本的に異なります。宗達の装飾的画風とは矛盾する写実的表現です。",
        realImage: "images/s9-q6-real.jpg",
        fakeImage: "images/s9-q6-fake.jpg",
      },
      {
        title: "桜花爛漫図",
        artist: "尾形光琳",
        year: "1710年",
        medium: "紙本着色",
        size: "162cm × 340cm",
        location: "京都個人蔵",
        description:
          "満開の桜を一本一本の花びらまで植物図鑑のように精密に描いた写実画。科学的な正確さで光と影を再現し、西洋的な空気遠近法で奥行きを表現している。",
        isAuthentic: false,
        hint: "光琳が自然をどのように表現したか思い出してください。",
        explanation:
          "贋作です。光琳は自然を「意匠化」「デザイン化」する画家であり、「植物図鑑のような精密さ」「科学的な正確さ」「空気遠近法」は琳派の美意識と正反対です。琳派は自然の本質をパターンと装飾に昇華させることを目指しました。",
        colors: ["#FFB6C1", "#228B22", "#FFDAB9", "#90EE90", "#DEB887"],
        fakeExplanation:
          "描写が写実的すぎ、光琳の自然を意匠化・デザイン化する琳派の美意識と正反対の表現になっています。",
        realImage: "images/s9-q7-real.jpg",
        fakeImage: "images/s9-q7-fake.jpg",
      },
      {
        title: "月下秋草図",
        artist: "酒井抱一",
        year: "1820年",
        medium: "紙本金地着色",
        size: "155cm × 170cm",
        location: "名古屋個人蔵",
        description:
          "金地に秋の草花と月を描いた作品。太く力強い輪郭線で植物が描かれ、鮮烈な原色が大胆に塗り分けられている。激しい筆致と荒々しい表現が画面に緊張感を与えている。",
        isAuthentic: false,
        hint: "抱一の画風の本質を思い出してください。",
        explanation:
          "贋作です。抱一は繊細で優美な叙情性を特徴とする画家です。「太く力強い輪郭線」「鮮烈な原色」「激しい筆致」「荒々しい表現」は抱一の画風と正反対であり、むしろ表現主義的な特徴です。抱一の真作は繊細な色調と優美な線描が特徴です。",
        colors: ["#FF0000", "#FFD700", "#000000", "#FF4500", "#228B22"],
        fakeExplanation:
          "力強い輪郭線や原色の使用が、抱一の繊細で優美な叙情性とは正反対の表現主義的な特徴を示しています。",
        realImage: "images/s9-q8-real.jpg",
        fakeImage: "images/s9-q8-fake.jpg",
      },
      {
        title: "四季花鳥図",
        artist: "鈴木其一",
        year: "1848年",
        medium: "紙本墨画",
        size: "各145cm × 55cm",
        location: "ジュネーブ個人蔵",
        description:
          "四季の花と鳥を墨の濃淡のみで描いた水墨画。禅の精神を反映した簡素な構図で、余白が「無」を表現している。中国の文人画の影響が色濃く、書と画が一体となった作品。",
        isAuthentic: false,
        hint: "琳派の視覚的特徴と、この作品の描写方法を比較してください。",
        explanation:
          "贋作です。琳派は金銀地に鮮やかな岩絵具の色彩と装飾的な構成を最大の特徴とします。「墨の濃淡のみ」「禅の精神」「文人画の影響」は琳派ではなく、南画（文人画）の特徴であり、画派の帰属が完全に異なります。",
        colors: ["#2F4F4F", "#696969", "#808080", "#A9A9A9", "#F5F5DC"],
        fakeExplanation:
          "墨画による文人画的表現が、琳派の金銀地に鮮やかな岩絵具の色彩と装飾的構成という本質的特徴と完全に矛盾しています。",
        realImage: "images/s9-q9-real.jpg",
        fakeImage: "images/s9-q9-fake.jpg",
      },
    ],
  },
];

// ランクデータ
const RANKS = [
  { min: 0, max: 2, name: "贋作師", description: "まだまだ修行が必要です…" },
  {
    min: 3,
    max: 4,
    name: "見習い鑑定士",
    description: "鑑定の基礎が身についてきました",
  },
  {
    min: 5,
    max: 6,
    name: "鑑定士",
    description: "なかなかの目利きですね！",
  },
  {
    min: 7,
    max: 8,
    name: "目利き",
    description: "優れた鑑定眼の持ち主です！",
  },
  {
    min: 9,
    max: 9,
    name: "真贋の達人",
    description: "完璧な鑑定眼！あなたは真の達人です！",
  },
];

function getRank(score) {
  return RANKS.find((r) => score >= r.min && score <= r.max);
}
